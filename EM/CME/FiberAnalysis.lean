...
/-!
# CME Fiber Analysis: Vanishing Conditional Bias, Fiber Decomposition, Transitions

Material moved from `EM.LargeSieve.WalkAnalysis` (sections §84-§86).
...
* `norm_sub_one_sq_eq`, `unit_norm_re_le_of_dist` : norm-one geometry helpers (PROVED)
* `vcb_ped_implies_ccsb`     : VCB + PED → CCSB (PROVED)
* `vcb_ped_implies_mc`       : VCB + PED → MC (PROVED)
* `cme_fiber_residue_class`  : walkZ(q,n) = a ⇒ euclid number ≡ a+1 mod q (PROVED)
* `fiber_mult_sum_decomposition` : M_N = ∑_a F(a) (PROVED)
* `walk_char_sum_via_visit_count` : S_N = ∑_a χ(a)·V(a) (PROVED)
* `cme_fiber_energy_bound`   : CME ⇒ ∑_a ‖F(a)‖² = o(N²) (PROVED)
* `cme_iff_fiber_equidist`   : CME ↔ all fiber sums o(N) (PROVED)
* `transitionCount`, `emVisitCount` : empirical transition/visit counts (defs)
* `transitionCount_eq_mult_fiber`, `transitionCount_row_sum`,
  `transition_char_sum_eq_fiber` : transition matrix structure (PROVED)
* `cme_iff_transition_char_vanish` : CME ↔ transition char sums vanish (PROVED)

No new open hypotheses are introduced in this file.
-/
...
* `VanishingConditionalBias` : the VCB hypothesis (def)
* `cme_implies_vcb`         : CME → VCB (PROVED: take μ = 0)
* `vcb_ped_implies_ccsb`    : VCB + PED → CCSB (PROVED: the two-case argument —
    μ far from 1 → direct bound; μ ≈ 1 → PED contradiction)
* `vcb_ped_implies_mc`      : VCB + PED → MC (PROVED, via CCSB)
-/
...
/-- **VanishingConditionalBias**: for any prime q not in the EM sequence and any
    ...
    Combined with PED, VCB implies CCSB (proved below as `vcb_ped_implies_ccsb`).
...
* `fiber_mult_sum_decomposition`   : M_N = ∑_a F(a)
* `walk_char_sum_via_visit_count`  : S_N = ∑_a χ(a)·V(a)
