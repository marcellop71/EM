...
5. **Stopping-time perspective is a reformulation**: Viewing seq(n+1) as the
   ...
   content beyond EM/Equidist/Bootstrap.lean.
...
Cross-references: EM/Meta/MarkovSieve.lean (Doeblin = CME),
EM/Group/CyclicWalkCoverage.lean (step != walk), EM/Equidist/Bootstrap.lean (sieve gap).
...
by mcBelow_implies_seq_ge in EM/Equidist/Threshold.lean. -/
...
/-- **The sieve gap forces large multipliers.**
    ...
    This is mcBelow_implies_seq_ge in EM/Equidist/Threshold.lean: if MC holds
...
The Z/4Z counterexample from EM/Group/CyclicWalkCoverage.lean is decisive:
...
/-- **Definitive landscape of the Dobrushin/stopping-time approach.**
    ...
    5. SE does not imply MUB: generation is algebraic, MUB is analytic,
       Z/4Z counterexample from EM/Group/CyclicWalkCoverage.lean shows the gap.
...
5. **SE does not bridge to MUB**: Subgroup Escape (SE) gives algebraic generation
   ...
   walk-position fiber. The Z/4Z counterexample (EM/Group/CyclicWalkCoverage.lean) is
...
**Cross-references**:
- EM/Meta/MarkovSieve.lean: DoeblinConvergenceForEM = CME (rfl),
  SieveOrbitControl = CCSB (rfl), SpectralGapForEMAverage
- EM/Group/CyclicWalkCoverage.lean: alternating_walk_misses_two (Z/4Z counterexample)
- EM/Equidist/Bootstrap.lean: prime_residue_escape (SE proved); EM/Equidist/Threshold.lean: mcBelow_implies_seq_ge
- EM/CME/Reduction.lean: ConditionalMultiplierEquidist (CME definition)
- EM/Adelic/Equidist.lean: cme_iff_adelic (CME decomposes as MWI + MME)
