...
theorem minFac_not_multiplicative :
  ...
  push Not
  exact ⟨6, 35, by omega, by omega, by norm_num, by norm_num⟩
...
/-- **Definitive landscape of Markov chain and sieve routes to MC.**
    ...
    The key facts are (routes through DSL / QuantitativeDSL archived 2026-08-17,
    Dead End #160: PopulationEquidist is false, so DSL is vacuous):
    1. CME implies MC (proved: CME -> CCSB -> MC)
    2. SieveOrbitControl = ComplexCharSumBound (definitional collapse)
    3. DoeblinConvergenceForEM = CME (definitional collapse)
    4. SpectralGap implies Doeblin = CME (exponential implies sublinear)
    5. Adelic decomposition: CME decomposes as MWI + MME

    No new route to MC is opened by the Markov chain or sieve approaches.
    The sole remaining gap is CME. -/
theorem markov_sieve_landscape :
    -- Route 1: CME directly implies MC
    (ConditionalMultiplierEquidist → MullinConjecture) ∧
    -- Route 2: SieveOrbitControl = ComplexCharSumBound (collapse)
    (SieveOrbitControl = ComplexCharSumBound) ∧
    -- Route 3: DoeblinConvergenceForEM = CME (collapse)
    (DoeblinConvergenceForEM = ConditionalMultiplierEquidist) ∧
    -- Route 4: SpectralGap -> CME (strictly stronger)
    (SpectralGapForEMAverage → ConditionalMultiplierEquidist) ∧
    -- Route 5: SieveOrbitControl -> MC (via CCSB -> MC)
    (SieveOrbitControl → MullinConjecture) :=
  ⟨cme_implies_mc,
   sieve_orbit_control_eq_ccsb,
   doeblin_eq_cme,
