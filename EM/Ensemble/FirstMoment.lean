...
* **Part 14c** (from `MixedEnsemble.lean`): `sqfreeCount_ge_quarter_real` /
  `sqfreeCount_ge_quarter_nat` — the squarefree count is at least X/4.
* **Part B** (formerly `FirstMomentB.lean`): k≥1 infrastructure and mod-6
  density —
  - `K1Infrastructure`: genSeq(n,1)=3 for n ≡ 1 mod 6 squarefree, factor-3 injection
  - `K2Infrastructure`: genSeq(n,2)=5 for n ≡ 19 mod 30 squarefree (CRT)
...
theorem buchstabWeight_two : buchstabWeight 2 = 2 / 3 := by
  ...
  have : (Finset.range 2).filter Nat.Prime = ∅ := by decide
...
```
FirstMomentStep + VarianceBound
  → EnsembleConcentration (from EM/Population/TransferStrategy.lean)
...
-- first_moment_step_implies_positive_density_rsd archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #9)
-- weak_mc_landscape archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #8, #9)
...
theorem kappaPartial_three : kappaPartial 3 = 1 / 3 := by
  ...
  have hfilt : (Finset.range 3).filter Nat.Prime = {2} := by decide
...
-- smlb_implies_positive_density_rsd archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #7)
...
-- lmg_implies_fmd archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #8)
-- fms_implies_fmd archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #9)
-- smlb_implies_fmd archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #7)
...
-- smlb_implies_decaying_smlb archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #7)
-- divergence_hierarchy archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #7, #8)
...
-- fms_kappa_ge_quarter archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #9)

end FMSKappaLowerBound

-- parity_smlb_landscape archived to EM/Archive/Ensemble/FirstMomentArchive.lean (RED #7, #9)

/-! ## Part 14c: Squarefree Count Lower Bound

Moved here from `EM/Ensemble/MixedEnsemble.lean` (historical Part-number label
kept): elementary lower-bound facts about `sqfreeCount`, which is defined in
this file. Used by the weak-FMCD chain (`weak_fmcd_proved`) and published in
the registry. -/
...
/-! ## Part B (formerly FirstMomentB.lean): k>=1 Infrastructure and Mod-6 Density -/
