...
/-! The Parseval identities for `ZMod.dft` (`stdAddChar_sum_eq`, `zmod_dft_parseval_complex`,
`zmod_dft_parseval`) now live in `EM/ForMathlib/ZModDftParseval.lean` (Mathlib-only; extracted
2026-08-18). -/
...
theorem mulChar_norm_one_of_unit (χ : MulChar (ZMod p) ℂ) (a : (ZMod p)ˣ) :
  ...
  rw [← MulChar.coe_equivToUnitHom χ a]
  exact hnorm
