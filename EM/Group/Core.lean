...
EM/Core/Defs.lean in algebraic terms.
...
/-- A prime is its own least factor (home-grown `Euclid.minFac`). -/
theorem minFac_eq_self_of_isPrime {n : Nat} (hp : IsPrime n) : Euclid.minFac n = n := by
  have h2 : 2 ≤ n := hp.1
  rcases hp.2 (Euclid.minFac n) (Euclid.minFac_dvd n h2) with h | h
  · have := (Euclid.minFac_isPrime n h2).1; omega
  · exact h
...
/-- The seventh term: `seq 7 = 6221671`; `prod 6 + 1 = 6221671` is prime (Mathlib's `norm_num`
primality certificate; kernel-checked, no `native_decide`). -/
theorem seq_seven : seq 7 = 6221671 := by
  ...
  rw [seq_succ, hprod6]
  exact MullinGroup.minFac_eq_self_of_isPrime
    ((MullinGroup.isPrime_iff_natPrime _).mpr (by norm_num))
...
theorem prefix_product_in_subgroup {q : Nat} [Fact (Nat.Prime q)] {n : Nat}
...
We lift the pigeonhole-based collision theorem from EM/Core/Defs.lean to the
...
The chain of implications:
    ...
    HH → MullinConjecture           (proved in EM/Core/Defs.lean)
