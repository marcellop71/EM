...
/-!
# Tree Sieve Decay: Block Coverage, Regeneration, and TSD Composition

## Overview

This file (Parts 7-12 and 14 of the original interpolation MC development,
"Layer 2") develops the block-coverage / regeneration machinery for iterated
hitting, defines the `TreeSieveDecay` (TSD) hypothesis, and composes TSD into
`MixedMC` and `MixedMullinConjecture`.

## Contents

* Part 7: Block coverage and iterated hitting -- `GoodAccumulator`, `Regeneration`,
  `block_coverage`, `good_accumulator_neg_one_reachable`,
  `good_accumulator_implies_capture`, `concat_prefix`, `concat_walk_prefix`,
  `concat_walk_tail`, `concat_valid`, `regeneration_implies_cofinal_hitting`,
  `regeneration_implies_capture_at_every_step`,
  `regeneration_implies_iterated_hitting`, `layer2_landscape`
* Part 8: TreeSieveDecay and bridge to regeneration -- `mixedWalkProd_squarefree`,
  `TreeSieveDecay`, `TreeSieveDecayHitting`, `mixedWalkProd_coprime_of_no_death`,
  `treeSieveDecay_implies_good_at`, `tsd_implies_neg_one_reachable`,
  `tsd_implies_tsdh`, `tsd_implies_good`, `tsd_implies_capture`,
  `full_interpolation_landscape`
* Part 9: Accumulator commutativity (orbit melting) --
  `eq_of_same_primeFactors_squarefree`, `same_accumulator_same_future`,
  `tail_reachable_implies_reachable`, `good_accumulator_propagates_to_start`,
  `orbit_melting_landscape`
* Part 10: TSD-Hitting(3) unconditional -- `tsd_hitting_three_unconditional`,
  `tsd_hitting_three_landscape`
* Part 11: TSD -> MixedMC composition -- `mixedWalkProd_exp_growth`,
  `mixedWalkProd_eventually_large`, `tsd_implies_reachable_from_any`,
  `tsd_implies_reachable_from_two`, `tsd_implies_mixed_mc_single`,
  `tsd_all_implies_mixed_mc`, `tsd_mixed_mc_landscape`
* Part 12: Coset ambiguity gap -- `coset_ambiguity_counterexample`,
  `single_coset_implies_immediate_hit`, `two_cosets_counterexample_five`,
  `coset_ambiguity_landscape`
* Part 14: Reachable set monotonicity -- `reachableEver_mono_along_walk`
-/
...
private theorem neg_one_ne_zero_zmod (q : ℕ) (hq : Nat.Prime q) (_hq2 : 2 < q) :
    (-1 : ZMod q) ≠ 0 := by
  have : Fact q.Prime := ⟨hq⟩; exact neg_ne_zero.mpr one_ne_zero
...
/-- **Uniform one-step escape to a 3-opportunity**: if `P ≥ 2` is coprime to `3`
    and `3 ∤ P+1` (no capture opportunity for `3` at `P` itself), then some prime
    `f ∣ P+1` with `f ≠ 3` moves the walk to a child `P·f` that DOES have one:
    `3 ∣ P·f + 1`.

    Together with the trivial case `3 ∣ P+1`, this says the depth to a
    3-opportunity is at most `1` from EVERY accumulator — a uniform block depth,
    with no hypotheses. Public form of the `P ≡ 1 (mod 3)` branch of
    `tsd_hitting_three_pointwise`, used by `EM/Stochastic/ThreeAlmostSure.lean`. -/
theorem exists_three_opportunity_step (P : ℕ) (hP : 2 ≤ P) (hcop : Nat.Coprime P 3)
    (hno : ¬(3 ∣ P + 1)) :
    ∃ f : ℕ, f.Prime ∧ f ∣ P + 1 ∧ f ≠ 3 ∧ 3 ∣ P * f + 1 := by
  rcases zmod3_nonzero_cases (ne_zero_zmod3_of_coprime hcop) with hmod1 | hmod2
  · obtain ⟨f, hprime, hfdvd, hfmod⟩ := exists_factor_of_succ_mod3 P hP hmod1
    have hf3 : f ≠ 3 := by
      intro h
      rw [h] at hfmod
      exact absurd hfmod (by decide)
    refine ⟨f, hprime, hfdvd, hf3, ?_⟩
    rw [← ZMod.natCast_eq_zero_iff]
    push_cast
    rw [hmod1, hfmod]
    decide
  · -- P ≡ 2 (mod 3) would give 3 ∣ P + 1, contradicting `hno`.
    exact absurd (by
      rw [← ZMod.natCast_eq_zero_iff]; push_cast; rw [hmod2]; decide) hno

/-- **Pointwise TSD-Hitting(3)**: EVERY integer P >= 2 coprime to 3 has
    (-1 : ZMod 3) in its reachable set — no squarefreeness and no size
    threshold needed. This is the pointwise content behind
...
theorem tsd_hitting_three_pointwise (P : ℕ) (hP : 2 ≤ P) (hcop : Nat.Coprime P 3) :
    (-1 : ZMod 3) ∈ reachableEver 3 P := by
  -- Case split on (P : ZMod 3)
...
/-- **TreeSieveDecayHitting(3) Unconditional**: Every squarefree integer P >= 2
    coprime to 3 has (-1 : ZMod 3) in its reachable set. Packaged form of
    `tsd_hitting_three_pointwise` (which needs neither squarefreeness nor the
    threshold). -/
theorem tsd_hitting_three_unconditional : TreeSieveDecayHitting 3 :=
  ⟨2, fun P hP _hsf hcop => tsd_hitting_three_pointwise P hP hcop⟩
