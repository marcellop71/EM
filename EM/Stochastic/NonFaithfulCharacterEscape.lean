...
/-!
# Non-Faithful Character Escape (NFCE)

## Overview

This is the NFCE file: it contains Parts 25-28 of the vanishing noise
framework (formerly "Vanishing Noise Variant Part D", split from
...
**Part 28**: Quotient Character Lift and NFCE Reduction. Non-faithful
characters factor through a quotient on which they are faithful, reducing
NFCE to faithful escape on quotient groups.
-/
...
theorem units_zmod_five_card :
  ...
  let : Fact (Nat.Prime 5) := Fact.mk (by decide)
...
/-- In a finite group, zpowers of elements with coprime orders have trivial intersection.
    This is a direct application of `Subgroup.disjoint_of_coprime_natCard`. -/
...
private theorem exists_nonfaithful_separating_char
  ...
  have : Fintype (G ⧸ H) := Quotient.fintype _
  have : DecidableEq (G ⧸ H) := Classical.decEq _
  -- exponent of (G/H)^x divides |G/H| which is finite
  have : NeZero (Monoid.exponent (G ⧸ H)ˣ : ℂ) := by
    constructor
...
theorem total_confinement_eventually_in_all_kernels
  ...
  have : Fintype ((ZMod q)ˣ →* ℂˣ) := Fintype.ofFinite _
