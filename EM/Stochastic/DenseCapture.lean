...
private theorem cylinder_isOpen (σ : SelectionSeq) (k : ℕ) :
  ...
  have heq : {τ : SelectionSeq | ∀ i, i ≤ k → τ i = σ i} =
    ...
    ext τ
    simp only [Set.mem_ofPred_eq, Set.mem_pi, Finset.coe_Icc, Set.mem_Icc, Set.mem_singleton_iff]
    constructor
...
theorem captureSet_level_isOpen (q acc : ℕ) (k : ℕ) :
  ...
  · intro τ hτ
    simp only [Set.mem_ofPred_eq] at hτ hσ ⊢
...
theorem captureSet_isOpen (q acc : ℕ) :
  ...
  have heq : captureSet q acc = ⋃ k, {σ | epsWalkFactorFrom acc σ k = q} := by
    ext σ; simp only [captureSet, Set.mem_ofPred_eq, Set.mem_iUnion]
  rw [heq]
...
theorem fullCapture_residual (acc : ℕ)
  ...
  have h_isGδ : IsGδ (⋂₀ ((captureSet · acc) '' {q : ℕ | q.Prime})) :=
    IsGδ.sInter (fun s hs => by
      simp only [Set.mem_image, Set.mem_ofPred_eq] at hs
      obtain ⟨q, _, rfl⟩ := hs
  ...
  have h_eq : ⋂₀ ((captureSet · acc) '' {q : ℕ | q.Prime}) =
    ...
    ext σ
    simp only [Set.mem_sInter, Set.mem_image, Set.mem_ofPred_eq, Set.mem_iInter]
    constructor
...
theorem fullCapture_exists_residual (acc : ℕ)
  ...
  intro σ hσ q hq
  simp only [Set.mem_iInter, Set.mem_ofPred_eq] at hσ
  exact hσ q hq
