...
This file contains Part 20 of the vanishing noise framework, split from
`VanishingNoise.lean` for modularity. (Part 21, the non-self-consistent
variant MC under `UFDStrong`, lives in `VanishingNoiseVariantB.lean`.)
