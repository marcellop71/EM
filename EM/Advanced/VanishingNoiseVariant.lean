...
theorem nonfaithful_ker_card_eq_two_of_order_four
  ...
  -- ker ≠ bot (not injective)
  have hker_ne_bot : chi.ker ≠ ⊥ := by rwa [Ne, MonoidHom.ker_eq_bot_iff]
  -- ker ≠ top (nontrivial)
  have hker_ne_top : chi.ker ≠ ⊤ := by rwa [Ne, MonoidHom.ker_eq_top_iff]
  -- Lagrange: |ker| · index = 4
  have hlag := chi.ker.card_mul_index
  rw [hcard] at hlag
  -- index ≥ 2
  have hidx_ge : 2 ≤ chi.ker.index :=
    chi.ker.one_lt_index_of_ne_top hker_ne_top
  -- |ker| ≥ 2
  have hker_ge : 2 ≤ Nat.card chi.ker := by
    have h1 : 1 < Nat.card chi.ker :=
      chi.ker.one_lt_card_iff_ne_bot.mpr hker_ne_bot
    omega
  -- From |ker| · index = 4 with both ≥ 2, both must be 2
  -- omega can't handle nonlinear; use have + interval_cases-style
  have hle_ker : Nat.card chi.ker ≤ 2 := by
    by_contra h
    push_neg at h
    have : 3 * 2 ≤ Nat.card chi.ker * chi.ker.index := Nat.mul_le_mul h hidx_ge
    omega
  omega
...
theorem nonfaithful_ker_index_eq_two_of_order_four
  ...
  have hker_ge : 2 ≤ Nat.card chi.ker := by
    have h1 : 1 < Nat.card chi.ker :=
      chi.ker.one_lt_card_iff_ne_bot.mpr hker_ne_bot
    omega
  have hle_idx : chi.ker.index ≤ 2 := by
    by_contra h
    push_neg at h
    have : 2 * 3 ≤ Nat.card chi.ker * chi.ker.index := Nat.mul_le_mul hker_ge h
    omega
  omega
...
theorem nfce_five_landscape :
    ...
    (letI := Fact.mk (by decide : Nat.Prime 5);
     ...
     ∀ (a : (ZMod 5)ˣ), ∃ N,
       0 < Multiset.count a (productMultiset (paddedUnitSet (q := 5)) N)) := by
  exact ⟨units_zmod_five_card,
         fun chi hchi hnf => nonfaithful_ker_card_two_at_five chi hchi hnf,
         ratio_escape_implies_nfce_five,
         fun h => variant_mc_five_of_ratio_escape h⟩
...
theorem intersection_kernel_landscape :
    ...
    (∀ (q' : ℕ) [Fact (Nat.Prime q')],
      ...
      TotalKernelConfinement q' →
      ∃ N₀, ∀ n, N₀ ≤ n → factorRatio (q := q') n = 1) :=
  ⟨fun _G _ _ x₁ x₂ hcop => coprime_order_zpowers_inf_bot hcop,
   fun _q' _ hfail hcof => total_nfce_failure_implies_confinement hfail hcof,
   fun _q' _ hsep htotal => total_confinement_implies_ratio_one hsep htotal⟩
