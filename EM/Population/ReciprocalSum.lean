...
**RED status (Session 288)**: `LinearMeanGrowth` is RED #8 — likely false (Session 201:
E[S_K] ≈ C·log log K, not κ·K). All theorems taking LMG as input are KEEP-conditional:
valid proved implications whose hypothesis is unlikely to hold.
See `audit/lmg_resolution.md` for the full resolution.
-/
...
The core definitions (`genProd`, `genSeq`) and their elementary structural
properties live upstream in `EM/Ensemble/GenEM.lean` (extracted from here,
Session 301 reorg). This section keeps only the population-facing bridge. -/
...
**RED status note (Session 288)**: `LinearMeanGrowth` is RED #8 — likely false
(Session 201: E[S_K] ≈ C·log log K under correct heuristics, not κ·K). All theorems
below that take LMG as a hypothesis are valid conditional mathematics: the implications
are proved, but the hypothesis is unlikely to hold. See `audit/lmg_resolution.md`. -/
...
**RED #8** (likely false, Session 201): under correct heuristics, E[S_K] grows as
C·log log K, not linearly. This definition is kept live because `ChebyshevConcentration`
mentions it as a hypothesis parameter. All theorems taking LMG as input are valid
conditional mathematics. -/
def LinearMeanGrowth : Prop :=
  ∃ κ : ℝ, 0 < κ ∧ ∀ K : ℕ, ∃ X₀ : ℕ, ∀ X ≥ X₀,
    κ * K ≤ sfAvg X (fun n => recipPartialSum n K)

-- linear_mean_growth_implies_emd archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)

/-- LMG + Var = O(K) implies RecipSumConcentration. PROVED.

Note: The `LinearMeanGrowth` hypothesis is RED #8 (likely false, Session 201).
This conditional statement remains valid mathematics — the implication is proved
by `chebyshev_concentration_proved` regardless of whether LMG holds. -/
...
-- psd_chain_implies_concentration archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_chain_implies_rsd archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_implies_rsd_with_bridges archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_chain_implies_concentration' archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_implies_rsd' archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_lmg_implies_concentration archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- psd_lmg_implies_rsd archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
...
Note: `lmg_implies_positive_density_rsd` takes LMG (RED #8) as hypothesis.
Valid conditional result — PROVED. -/
...
-- density_lower_bound_from_mean archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- lmg_implies_positive_density_rsd archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)

end PositiveDensityRoute

/-! ## Landscape

All three bridges PROVED. Open: PSD. -/

section Landscape

-- dead_end_125_pairwise_revival archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
-- positive_density_landscape archived to EM/Archive/Population/ReciprocalSumArchive.lean (RED #8)
