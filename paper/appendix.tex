...
%% =========================================================================
\section{Weak Mullin and Confined Energy}
...
$\mathbb{P} = A \cup M$,
where $A = \{\seq(k) : k \in \NN\}$ is the set of appeared primes.
...
Suppose for contradiction that $\sum_k 1/\seq(k)$ converges.  Since
$\seq$ is injective (the EM sequence never repeats a prime), the map
$k \mapsto \seq(k)$ is an injection into~$A$, so $\Sigma_A$ converges.
...
the sequence diverges.  Contrapositively, if $\sum 1/\seq(k)$
...
\]
where $V_N(a) = |\{n < N : \walkZ{q}{n} = a\}|$ is the visit count.
\end{theorem}
...
\begin{definition}[\defname{JointSVE}]
...
The EM accumulator $\Prod(n)$ is a product of distinct primes and is
therefore squarefree:

\begin{theorem}[\lean{EM/WeakErgodicity.lean}{prod\_squarefree}]
$\Prod(n)$ is squarefree for all~$n$.
\end{theorem}

\begin{proof}
By induction.  The base case $\Prod(0) = 2$ is prime, hence
squarefree.  For the inductive step, $\Prod(n{+}1) = \Prod(n) \cdot
\seq(n{+}1)$.  The new factor $\seq(n{+}1)$ divides $\Prod(n)+1$
and is therefore coprime to~$\Prod(n)$ (consecutive integers share no
common factor).  Since both $\Prod(n)$ (by induction) and $\seq(n{+}1)$
(a prime) are squarefree, and they are coprime, their product is
squarefree.
\end{proof}

Consequently, every Euclid number $\Prod(n)+1$ belongs to the
...
\abbr{RD} & ReciprocalDivergence
    & $\sum_k 1/\seq(k) = \infty$; WM $\Rightarrow$ RD (proved)
    & App.~\ref{app:weak-mullin} \\
...
--- & Squarefree Accumulator
    & $\Prod(n)$ is squarefree (product of distinct primes); proved
    & App.~\ref{app:weak-mullin} \\
--- & QR Obstruction
