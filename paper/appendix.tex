...
$\operatorname{gpf}(\Prod{2}(n) + 1)$ instead of $\minFac$---provably
...
integers to the specific subsequence $\{\Prod{2}(n)+1\}_{n \geq 0}$.
...
\begin{theorem}[\lean{EM/LargeSieve/Basic.lean\#L403}{arith\_ls\_chain\_mc}]
...
\begin{theorem}[\lean{EM/LargeSieve/Analytic.lean\#L490}{weak\_als\_from\_card\_bound}]
\label{thm:weak-als}
...
\begin{theorem}[\lean{EM/LargeSieve/Analytic.lean\#L304}{char\_sum\_to\_exp\_sum}]
...
\begin{theorem}[\lean{EM/LargeSieve/Analytic.lean\#L1358}{als\_implies\_prime\_arith\_ls}]
\label{thm:als-prime}
$\mathrm{AnalyticLargeSieve} \;\Longrightarrow\; \mathrm{PrimeArithLS}$.
\end{theorem}

\begin{theorem}[\lean{EM/LargeSieve/Analytic.lean\#L1535}{als\_prime\_arith\_ls\_chain\_mc}]
$\mathrm{ALS} + \mathrm{PrimeArithLSImpliesMMCSB} \;\Longrightarrow\; \MC$.
\end{theorem}

A deeper decomposition of ALS itself is architectured in
\code{IKCh7.lean}: the \textbf{Hilbert chain}
\[
...
\mathrm{ALS}
\]
(\lean{EM/IKCh7.lean\#L1467}{hilbert\_chain\_als}).
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean\#L89}{walk\_energy\_parseval}]
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean\#L238}{sve\_implies\_mmcsb}]
\label{thm:sve-mc}
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean\#L826}{van\_der\_corput\_bound}]
\label{thm:vdc}
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean\#L1038}{hod\_implies\_ccsb}]
\label{thm:hod-mc}
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean}{cme\_implies\_dec}]
$\mathrm{CME} \Longrightarrow \mathrm{DecorrelationHypothesis}$.
\end{theorem}

\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean}{cme\_implies\_ccsb}]
\label{thm:cme-ccsb}
...
\begin{theorem}[\lean{EM/LargeSieve/Spectral.lean}{cme\_chain\_mc}]
...
%% =========================================================================
\subsection{Weak Mullin and Confined Energy}
\label{app:weak-mullin}
%% =========================================================================

The formalization explores what can be said about the ``missing
primes''---those primes that never appear in the EM sequence---without
assuming MC outright.
...
\begin{proof}
Partition the primes into two disjoint sets:
$\mathbb{P} = A \cup M$,
where $A = \{\seq{2}(k) : k \in \NN\}$ is the set of appeared primes.
The sets are disjoint since every missing prime is, by definition, not
in~$A$.  Consider the indicator sums
$\Sigma_A = \sum_{p \in A} 1/p$ and
$\Sigma_M = \sum_{q \in M} 1/q$.

Suppose for contradiction that $\sum_k 1/\seq{2}(k)$ converges.  Since
$\seq{2}$ is injective (the EM sequence never repeats a prime), the map
$k \mapsto \seq{2}(k)$ is an injection into~$A$, so $\Sigma_A$ converges.
WM gives that $\Sigma_M$ converges.  But then
$\sum_{p \in \mathbb{P}} 1/p = \Sigma_A + \Sigma_M$ converges,
contradicting the divergence of the sum of prime
reciprocals~\textup(\texttt{not\_summable\_one\_div\_on\_primes} in
Mathlib\textup).
\end{proof}

The chain $\MC \Rightarrow \mathrm{WM} \Rightarrow \mathrm{RD}$ gives a proved
consequence of MC: if every prime appears, then the reciprocal sum of
the sequence diverges.  Contrapositively, if $\sum 1/\seq{2}(k)$
converged, some primes would be missing with divergent reciprocal
sum---i.e., the missing set would be ``large'' in the harmonic sense.

\paragraph{Product divergence.}
The negation of WM has a quantitative consequence for products over
missing primes.

\begin{theorem}[\lean{EM/Population/WeakMullin.lean}{safe\_fraction\_exp\_bound}]
For a finite set $Q$ of reals with $q \geq 2$ for each $q \in Q$:
\[
\prod_{q \in Q} \frac{q-2}{q-1}
\;\leq\; \exp\!\Bigl(-\sum_{q \in Q} \frac{1}{q-1}\Bigr).
\]
\end{theorem}

\begin{proof}[Proof sketch]
Each factor satisfies $1 - 1/(q{-}1) \leq e^{-1/(q-1)}$ by the
inequality $1 - x \leq e^{-x}$.  Taking the product over~$Q$ gives
the result.
\end{proof}

\begin{theorem}[\lean{EM/Population/WeakMullin.lean}{not\_wm\_product\_diverges}]
If\/ $\neg\mathrm{WM}$, then for every $C \in \mathbb{R}$ there exists a finite
subset $F \subseteq M$ with
$\prod_{q \in F} \tfrac{q-1}{q-2} \geq C$.
\end{theorem}

\begin{proof}[Proof sketch]
$\neg\mathrm{WM}$ gives $\sum_{q \in M} 1/q = \infty$, hence
$\sum_{q \in M} 1/(q{-}1) = \infty$.  By the safe fraction bound,
$\prod_{q \in F} (q{-}2)/(q{-}1) \leq e^{-\sum 1/(q-1)}$, which
tends to~$0$ as~$F$ grows.  Its reciprocal
$\prod (q{-}1)/(q{-}2)$ therefore exceeds any bound.
\end{proof}

The product $\prod (q{-}1)/(q{-}2)$ over missing primes measures
the ``confinement cost'': each missing prime~$q$ confines the walk
to $q-2$ of the $q-1$ units in $(\ZZ/q\ZZ)^\times$ (avoiding~$-1$).
If WM fails, this confinement cost is unbounded, meaning that avoiding
more and more primes imposes an exponentially growing constraint.

\paragraph{EMBV and confined energy.}
The confined-energy perspective connects the missing primes to the
spectral route via visit counts.

\begin{property}[\defname{EMBV}]
\lean{EM/Population/WeakMullin.lean}{EMBV}
An alias for $\mathrm{MMCSB}$, emphasizing the
Bombieri--Vinogradov analogy: the EM walk satisfies multi-modular
character sum bounds.
\end{property}

\begin{theorem}[\lean{EM/Population/WeakMullin.lean}{embv\_implies\_mc}]
$\mathrm{EMBV} \Longrightarrow \MC$ \textup(conditional on the
sieve-gap threshold $Q_0 \leq 11$\textup).
\end{theorem}

For a missing prime $q \geq 3$, the walk avoids $-1$ entirely, so the
$N$ visits are distributed among only $q-2$ residues instead of the
full $q-1$.  A pigeonhole argument gives:

\begin{theorem}[\lean{EM/Population/WeakMullin.lean}{per\_prime\_confined\_energy\_lb}]
For a missing prime $q \geq 3$ and $N$ steps:
\[
\frac{N^2}{q-2}
\;\leq\;
(q{-}1) \sum_{a \in (\ZZ/q\ZZ)^\times} V_N(a)^2,
\]
where $V_N(a) = |\{n < N : \walkZ{2}{q}{n} = a\}|$ is the visit count.
\end{theorem}

\begin{proof}[Proof sketch]
Since $q$ is missing, $V_N(-1) = 0$, so all $N$ visits lie among
$q{-}2$ residues.  By Cauchy--Schwarz (or convexity of~$x^2$),
$\sum_a V_N(a)^2 \geq N^2/(q{-}2)$.  Multiplying by~$q{-}1$ gives
the stated bound.
\end{proof}

Compare this with the equidistributed baseline $N^2/(q{-}1)$: the
confined-to-equidistributed energy ratio is $(q{-}1)/(q{-}2)$, which
is bounded away from~$1$ for all~$q$.  This excess energy is precisely
what SVE forbids: subquadratic visit energy requires the walk to
spread evenly, but a missing prime forces concentration.

\begin{property}[\defname{JointSVE}]
\lean{EM/Population/WeakMullin.lean}{JointSVE}
Joint subquadratic visit energy: the sum of excess energies over any
finite set of missing primes is $o(N^2)$.
\end{property}

\begin{theorem}[\lean{EM/Population/WeakMullin.lean}{jointSVE\_implies\_mmcsb}]
$\mathrm{JointSVE} \Longrightarrow \mathrm{MMCSB}$.
\end{theorem}

The proof reduces JointSVE to per-prime SVE (by instantiating with
singleton sets), then applies the proved SVE~$\Rightarrow$~MMCSB
chain.  This connects the ``missing primes'' picture to the spectral
route: bounding the total confined energy across primes is equivalent
to the multi-modular character sum bound.

\paragraph{The shifted squarefree population.}
The EM accumulator $\Prod{2}(n)$ is a product of distinct primes and is
therefore squarefree:

\begin{theorem}[\lean{EM/Population/WeakErgodicity.lean}{prod\_squarefree}]
$\Prod{2}(n)$ is squarefree for all~$n$.
\end{theorem}

\begin{proof}
By induction.  The base case $\Prod{2}(0) = 2$ is prime, hence
squarefree.  For the inductive step, $\Prod{2}(n{+}1) = \Prod{2}(n) \cdot
\seq{2}(n{+}1)$.  The new factor $\seq{2}(n{+}1)$ divides $\Prod{2}(n)+1$
and is therefore coprime to~$\Prod{2}(n)$ (consecutive integers share no
common factor).  Since both $\Prod{2}(n)$ (by induction) and $\seq{2}(n{+}1)$
(a prime) are squarefree, and they are coprime, their product is
squarefree.
\end{proof}

Consequently, every Euclid number $\Prod{2}(n)+1$ belongs to the
\emph{shifted squarefree population}
$\mathcal{S} = \{m \geq 2 : m{-}1 \text{ squarefree}\}$, which has
density $6/\pi^2 \approx 0.608$ among the positive integers.

This gives a decomposition of WE~(= EMDirichlet) into two components:
\begin{itemize}[nosep]
\item \textbf{Population Equidistribution (PE):} among elements of
  $\mathcal{S}$, the smallest prime factor is equidistributed modulo
  any prime~$q$.  This is provable by the Selberg sieve and
  Dirichlet's theorem: the sieve weights
  $g(p) \prod_{r<p}(1{-}g(r))$ are smooth in~$p$ and do not depend
  on $p \bmod q$, so summing over primes in each residue class gives
  density $1/(q{-}1)$.
\item \textbf{Population Transfer (PT):} the EM trajectory samples
  $\mathcal{S}$ without introducing mod-$q$ bias.  Formally:
  $\mathrm{PE} \Rightarrow \mathrm{EMDirichlet}$.  This is the open
  content, structurally supported by
  \texttt{crt\_multiplier\_invariance} (the sampling rule is $q$-blind).
\end{itemize}

\begin{theorem}[\lean{EM/Population/WeakErgodicity.lean}{pe\_transfer\_cme\_implies\_mc}]
$\mathrm{PE} + \mathrm{PT} + \mathrm{EMDImpliesCME}
\;\Longrightarrow\; \MC$.
\end{theorem}

The chain is: PE~$+$~PT gives EMDirichlet, which with EMDImpliesCME
gives CME, which implies CCSB, which implies MC\@.  The open content
is isolated in PT (the trajectory-to-population transfer) and
EMDImpliesCME (the unconditional-to-conditional transfer).  Both gaps
are instances of the orbit-specificity barrier: transferring population
statistics to a specific deterministic trajectory.
...
136~potential approaches
were explored and found to be dead ends (catalogued in full in
\code{Meta/DeadEnds.lean}, with 13~formal Lean witnesses).
Each elimination is informative:
it narrows the space of viable strategies.  Eight dead ends have
weak-MC revival score~$\geq 2$, meaning they contribute to weaker forms
of the conjecture (see~\S\ref{sec:open}).
A representative selection, grouped by the type of obstruction:

\smallskip
\renewcommand{\arraystretch}{1.15}
\small
\begin{longtable}{@{}p{3.6cm}p{8.8cm}@{}}
\toprule
...
Information-theoretic bounds
  & Category error: entropy/mutual-information tools assume a
    ...
    entropy. \\
\midrule
\multicolumn{2}{@{}l}{\textsc{Multiplier-to-walk transfer}:
  \emph{multiplier cancellation does not force walk cancellation}} \\[2pt]
MultCancelToWalkCancel
  & Multiplier energy $o(K)$ does not force walk energy $o(K)$.
    Counterexample: alternating multipliers $\{2,3\} \bmod 5$
    with $\chi(2)=i$ give multiplier cancellation yet
    $|W_K| = \Theta(K)$.  Compatible with \emph{all} EM structural
    properties (squarefree accumulator, coprime cascade, CRT
    invariance). Dead End~\#117. \\
Cofactor identity
  & The ``$+1$ shift'' identity
    $w(n){+}1 = m(n) \cdot \mathrm{cofZ}(n)$
    (Theorem~\ref{thm:cofactor}) factorizes one correlated variable
    into two; the cofactor remains a joint quantity, so the
    Marginal/Joint Barrier persists. Dead End~\#115. \\
SelfCorrectingDrift
  & The Lyapunov function $L(N) = \sum_c (V_c(N) - N/(q{-}1))^2$
    telescopes to $L(N) = 2R(N) + O(N)$, so
    SCD ($R(N) = o(N^2)$) is algebraically equivalent to SVE
    ($L(N) = o(N^2)$).  SCD provides no new proof leverage; it
    is a dynamical reformulation, not a reduction.  Dead End~\#120. \\
SMSB $+$ SE per-class escape
  & SecondMomentSquaredBound gives a global bad-set density bound;
    combining with SubgroupEscape to get per-class escape requires
    BadSetEquidistribution (bad steps don't concentrate in one class).
    BSE is a consequence of CME: going from marginal to conditional
    bad-density requires joint (position, badness) independence, which
    IS the orbit-specificity gap.  Pigeonhole gives ``some class'' but
    not the specific class~$-1$ needed for MC\@.  Dead End~\#121. \\
Dobrushin coefficient
  & The Dobrushin coefficient of ergodicity equals~$1$ for every step
    of the deterministic EM walk (each transition is a point mass;
    disjoint support gives overlap~$= 0$).  Dobrushin's theorem
    requires $\sum(1-\alpha) = \infty$, but $\sum 0 = 0$.
    The weaker MUB hypothesis is genuinely weaker than CME
    (CME $\Rightarrow$ MUB proved) but useless without Dobrushin.
    Dead End~\#131. \\
\midrule
  $L$-function factorization
  & The factorization $L(s,\chi) = L_{\mathrm{EM}}(s,\chi)\cdot L_{\mathrm{non\text{-}EM}}(s,\chi)$
    is circular: proving $L_{\mathrm{non\text{-}EM}} \neq 0$ on $\Re(s) = 1$ requires knowing
    which primes are non-EM, i.e., requires MC itself. Dead End~\#132. \\
\midrule
  Self-similar functional equation
  & The tail identity gives $L_{\mathrm{EM}} = \text{head} + L_{\mathrm{EM, from } P(M)}$,
    but the tail orbit starts from $P(M) \neq 2$, so it is \emph{not} a scalar multiple
    of $L_{\mathrm{EM}}$. The Lapidus--van Frankenhuijsen fractal zeta framework requires
    scaling ratios $\zeta(s) = \sum r_j^s \zeta(s) + f(s)$; the EM self-similarity
    does not match this form. Dead End~\#133. \\
\midrule
  No Tauberian lever
  & The super-exponential growth of $P(n)$ makes $\sum \mathrm{prod}(n)^{-s}$ converge
    for all $\Re(s) > 0$ (proved: \code{accum\_reciprocal\_summable}). The resulting
    Dirichlet series is entire in $\Re(s) > 0$ --- no pole at $s = 1$ to exploit
    via Tauberian methods. The standard PNT argument uses the pole of~$\zeta(s)$;
    $L_{\mathrm{EM}}$ has no such pole because EM primes are too sparse.
    Dead End~\#134. \\
\midrule
  Konyagin--Shparlinski
  & Character sums $\sum \chi(g^n)$ cancel when $\mathrm{ord}(g)$ is large
    (Konyagin--Shparlinski~\cite{KonyaginShparlinski1999}).  The EM walk position
    $\Prod{2}(n) \bmod q = 2 \cdot \seq{2}(1) \cdots \seq{2}(n) \bmod q$
    looks like $g^n$ when $g = 2$, and $2$ has large order mod most primes
    (Artin's conjecture).  Fatal gap: the walk is $2 \cdot \prod \seq{2}(k)$,
    not $2^n$.  The varying multipliers destroy the geometric progression
    structure that makes the Konyagin--Shparlinski bound work.
    Dead End~\#135. \\
\midrule
  Arithmetic of~$2$ (general)
  & The seed $x_0 = 2$ has special arithmetic: $\mathrm{ord}_q(2)$ is large for
    most primes~$q$ (Artin), and $2$ generates large subgroups.
    Every attempt to exploit this---Konyagin--Shparlinski, Artin--Hooley,
    power-residue structure---fails because the walk's varying multipliers
    break the fixed-generator structure.  The arithmetic of~$2$ is
    ``genuinely untried'' in the sense that no mechanism has been found to
    convert Artin-type information about $\mathrm{ord}_q(2)$ into walk
    equidistribution through a route that is \emph{not} Fourier $+$
    Cauchy--Schwarz.  Dead End~\#136. \\
\bottomrule
\end{longtable}
...
\abbr{ANT} & ---
    & Standard analytic number theory (Dirichlet + sieve $\Rightarrow$ MFRE);
      assumed, not formalized
    & \S\ref{sec:ant-def} \\
\abbr{MFRE} & MinFacResidueEquidist
    & $\minFac$ residues equidist.\ mod $q$ among $q$-rough sqfree ints;
      MFRE $\Rightarrow$ PE (proved)
    & \S\ref{sec:ant-def} \\
\midrule
...
\abbr{TWD} & TailWindowDecorrelation
    & Avg.\ $|$cross term$|$ between windows vanishes; TWD $\Rightarrow$ CCSB $\Rightarrow$ MC
    & \S\ref{sec:twd} \\
...
--- & JointSVE
    ...
    & App.~\ref{app:weak-mullin} \\
--- & PopulationEquidist
    & minFac equidist mod $q$ in shifted squarefree population (provable by sieve)
    & App.~\ref{app:weak-mullin} \\
--- & PopulationTransfer
    & PE $\Rightarrow$ EMDirichlet (open = unbiased sampling); PE+PT $\Rightarrow$ MC
    & App.~\ref{app:weak-mullin} \\
...
\abbr{DSL} & DeterministicStabilityLemma
    & Ensemble char energy concentrates over sqfree starting points; DSL $\Rightarrow$ MC
    & \S\ref{sec:ensemble} \\
--- & DSLHitting
    & Weaker: walk hits $-1$ for positive density of starting points; PE+DSLHitting $\Rightarrow$ MC
    & \S\ref{sec:ensemble} \\
--- & genProd / genSeq
...
--- & MinFacResidueEquidist
    & minFac residues equidist for $q$-rough sqfree ints; MFRE $\Rightarrow$ PE (proved)
    & \S\ref{sec:ensemble} \\
--- & JointStepEquidist
