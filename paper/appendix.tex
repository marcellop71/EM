...
it must eventually hit~$-1$.  The formalization's CME, CCSB, and DSL
...
137~potential approaches
were explored and found to be dead ends (catalogued in full in
\code{Meta/DeadEnds.lean}, with 14~formal Lean witnesses).
...
\midrule
  ...
  & AccumulatorEquidistPropagation (AEP) is heuristically false at all fixed primes~$q$.
    At $q = 3$, death ($\Prod{n}(k) \equiv -1 \bmod 3$) deterministically produces
    $\seq{n}(k) = 3$ (\code{genSeq\_eq\_three\_of\_genProd\_mod3}), sending the orbit
    to the absorbing state $0 \bmod 3$ forever.  The nonzero-class densities $F_k(a)$
    decay exponentially; the correct equidistribution limit $r/(r^2{-}1)$ is never
    reached for $k \geq 1$.  CRTPropagationStep inherits the falsity.
    The backward dynamics chain ETA+DCTA+SRE $\Rightarrow$ PRSD is vacuously true.
    Dead End~\#138. \\
\bottomrule
\end{longtable}
...
human author and an AI system (Claude, Anthropic) across 150+~sessions.
...
The human author guided the proof effort across 150+~sessions, suggesting
