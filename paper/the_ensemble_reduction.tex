...
\noindent This provides a clean decomposition: SRE is the base case (standard
analytic number theory, hereafter \emph{ANT}),
...
% --- Row 1: ANT ---
\node[provedbox] (ant) {ANT};

% --- Row 2: SRE ---
\node[below=of ant, provedbox] (sre) {SRE};

% --- Row 3: AccumEquidist ---
\node[below=of sre, provedbox] (ae) {AccumEquidist};

% --- Row 4: EME ---open--> DSL ---
\node[below=of ae, provedbox] (eme) {EME};
\node[right=2.8cm of eme, openbox] (dsl) {\textbf{DSL}};
...
% Ensemble column
\draw[proved] (ant) -- (sre);
\draw[proved] (sre) -- (ae) node[midway, right, font=\tiny] {CRT};
\draw[proved] (ae) -- (eme);
...
(ANT~$\to$~SRE~$\to$~AccumEquidist~$\to$~EME~$\dashrightarrow$~DSL~$\to$~CME~$\to$~CCSB~$\to$~hits~$\to$~MC)
