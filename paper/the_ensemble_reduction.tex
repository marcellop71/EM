...
\subsection{The Seed-Average Law and Almost-All Capture}
\label{sec:seed-average}
\keymark

The bag-conditioned law of \S\ref{sec:bag-law} describes \emph{one} step
...
seed captures~$q$, in natural density, at a finite horizon and with no
hypotheses at all.
...
natural density}, because natural density is only finitely additive and
the per-$q$ statements therefore do not combine.  Finite subadditivity
does give a uniform
statement over any \emph{finite} set~$S$ of primes --- one horizon~$n$ such
that the seeds missing \emph{some} $q \in S$ have upper density
$\le\varepsilon$
...
on a population --- the first in this project that is neither an
equivalence to MC nor conditional on an open hypothesis.
\end{remark}
...
the primes $r \le y$ split three ways: those in the bag (dividing the seed,
...
\[
  \#\bigl(\mathrm{cell} \cap \{\,\text{no prime } r \leq y,\ r \neq q,
    \text{ divides the next Euclid number}\,\}\bigr)
  \;=\; S \cdot \#\,\mathrm{cell},
  \qquad
...
This is an exact identity, not an estimate: the roughness survival product
is the true proportion, cell by cell.  It is Shanks' heuristic made exact
\emph{and conditional} --- conditional on the whole type of the orbit so
far, which is what a heuristic cannot be.  The bag-conditioned law of
\S\ref{sec:bag-law} is its $n=0$ case: with no steps taken the type is
...
\noindent Summing over the near band $r < 2n$ and using
$\sum_{r<2n} H_{r-1} \le \theta(2n) + \pi(2n) = O(n)$
(\lean{EM/Population/LargeStepRoughness.lean}{chargeBudget\_le}) gives a
...
There is an absolute $n_0$ such that for every prime~$q$, every seed~$m$,
every $Y$ with $\log Y \le n^2$ and every nondegenerate $Y$-bounded type
path of length $n \geq n_0$,
\[
  \sum_{k<n} S_k \;\geq\; \frac{c_1}{2}\, n,
  \qquad c_1 = e^{-250} .
\]
\end{theorem}
...
not depend on $q$, on the seed, on $Y$ or on the threshold constant.  This
...
\label{thm:lemma-D}
For every prime~$q$ and every $C \geq 48q$ there are
$\kappa = e^{-128}/\bigl(16\,\varphi(q)\bigr) > 0$ and a depth~$k_0$ such
that, at every depth $k \ge k_0$, in every type cell with nondegenerate
$Y$-bounded reference prefix and small seed-divisor mass in the exclusion
window, and for \emph{every} class~$a$ coprime to~$q$, the seeds of the
cell that take a large step at depth~$k$ land in the class $a$ with
proportion at least~$\kappa$.
\end{theorem}

The constant is uniform in the cell, in the type and in the seed; only its
positivity is used downstream.  The analytic input is the mass of a
\emph{window} of primes in a progression: over $(y, y^2]$ the reciprocal
...
Assembling: on one period $[1,M_Y]$, the \emph{good uncaptured} seeds ---
...
\]
(\lean{EM/Population/TheoremC.lean}{theorem\_C}) by a single application of
\lean{EM/Population/TreeChernoff.lean}{chernoff\_quarter\_local}.  The
...
verbatim with the same $\kappa$, $K_0$, $n_1$
...
Two side remarks on the mechanics.  \emph{$q \le Y$ is load-bearing}: the
modulus deliberately omits~$q$, so the $q$-coordinate of the lift comes
from CRT, and if $q > Y$ the primes in $(Y,q)$ would be uncorrelated
between $x$ and $m'$, breaking the agreement exactly at the capture step.
...
\noindent The union over~$q$ is taken by countable additivity
(\code{measure\_iUnion\_null}); for each fixed~$q$ the horizon is sent to
