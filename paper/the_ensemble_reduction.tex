...
\item[\S\ref{sec:dsl-route}] The DSL reduction --- DSL $\Rightarrow$ CME $\Rightarrow$ CCSB $\Rightarrow$ MC, the main chain.
\item[\S\ref{sec:weyl-bridge}] Generalized MC and the Weyl hitting bridge --- character cancel $\Rightarrow$ walk hitting via strong induction.
\item[\S\ref{sec:visit-eqd}] Visit equidistribution and spectral identity --- SVE $\leftrightarrow$ VE, Lyapunov identity.
\end{enumerate}
\end{roadmap}
...
\begin{theorem}[\lean{EM/Ensemble/CRT.lean}{sre\_crt\_implies\_accum\_equidist}]
\label{thm:sre-crt}
...
(\code{Ensemble/CRTFreedom.lean}).

The CRT freedom framework proves:

\begin{theorem}[\lean{EM/Ensemble/CRTFreedom.lean}{scrti\_sre\_implies\_aep}]
...
(\lean{EM/Population/Proof.lean}{MinFacResidueEquidist}).
...
An investigation of the formalization gap
(\S\ref{sec:ant-formalization}) shows that ${\sim}92\%$ of the
required Mathlib infrastructure already exists (including Dirichlet's
theorem via $L$-series, Abel summation, and $L$-function
non-vanishing).  The one-sided Tauberian lemma, prime power stripping, and
the discrete Abel summation chain are now machine-verified
(\code{Population/Tauberian.lean}, 557~lines;
\code{Population/AbelChain.lean}, 651~lines), giving the full composed arrow
WeightedPNTinAP~$\Rightarrow$~PrimesEquidistributedInAP.
The Alladi chain from PrimesEquidistInAP to MFRE is decomposed
into two precisely stated open hypotheses with a proved master
reduction
(\code{Population/AlladiDensity.lean}, 275~lines).
The two remaining blocking dependencies are the
real-variable Wiener--Ikehara bridge
(converting $L$-series behavior near $s = 1$ into partial sum
asymptotics) and the Alladi density content
(sieve-theoretic proof of the two Alladi chain hypotheses).
...
ensemble mean.  The two threads converge in the DSL route
...
\begin{theorem}[\lean{EM/Ensemble/FirstMoment.lean}{first\_moment\_variance\_implies\_rsd}]
...
\begin{theorem}[\lean{EM/Ensemble/FirstMoment.lean}{first\_moment\_step\_implies\_positive\_density\_rsd}]
\label{thm:fms-pdrsd}
$\mathrm{FirstMomentStep}(\kappa) \;\Longrightarrow\;
\mathrm{PositiveDensityRSD}$, for any $\kappa > 0$.
\end{theorem}

\noindent The proof composes two machine-verified links:
$\mathrm{FirstMomentStep} \Rightarrow \mathrm{LMG}$
(\lean{EM/Ensemble/FirstMoment.lean}{first\_moment\_step\_implies\_lmg},
via $\mathbb{E}[\sum] = \sum\mathbb{E}$ and a finite-max threshold
argument) and $\mathrm{LMG} \Rightarrow \mathrm{PositiveDensityRSD}$
(the partition argument above).  FirstMomentStep is the sole
remaining hypothesis: it states that the ensemble average of
$1/\seq{n}(k)$ converges to~$\kappa$ for each~$k$, a standard
consequence of sieve theory for~$k=0$ and a mild orbit-transfer
for~$k \geq 1$.
...
(\lean{EM/Ensemble/FirstMoment.lean}{smlb\_implies\_positive\_density\_rsd}).
If FirstMomentStep$(\kappa)$ holds, then $\kappa \geq 1/4$
(\lean{EM/Ensemble/FirstMoment.lean}{fms\_kappa\_ge\_quarter},
...
(\lean{EM/Ensemble/CRT.lean}{accumMod3LB\_iff\_deathDensity3}).

A positive lower bound on the death density at any single
prime~$q \geq 3$ for all steps~$k$ yields the full chain:

\begin{theorem}[\lean{EM/Ensemble/CRT.lean}{death\_density\_implies\_prsd}]
...
\lean{EM/Ensemble/CRT.lean}{ewe\_landscape\_extended}.
...
to the absorbing class~$0$ (Dead End~\#138).
...
(\lean{EM/Ensemble/FirstMoment.lean}{smlb\_implies\_decaying\_smlb},
...
\paragraph{Conditional MFRE and the summability argument.}
A more refined approach avoids fixing a single prime~$q$.  The
\textbf{MFREConditional} hypothesis
(\lean{EM/Ensemble/CRT.lean}{MFREConditional})
states that for squarefree~$m$ with $m \equiv c \pmod{q}$
(where $c \neq 0, -1$), the conditional distribution
of $\minFac(m{+}1) \bmod q$ on each nonzero class converges
to $1/(q{-}1) + O(1/q^2)$, with the $O$-constant
depending only on~$q$.  By CRT, conditioning on~$m \bmod q$
constrains one coordinate; the remaining coordinates are
approximately independent for squarefree integers
(\code{crt\_multiplier\_invariance}).  The correction term
$g(q) - 1/q = 1/(q(q^2{-}1))$ (\code{sieveDensity\_sub\_inv})
gives the error scale.

The orbit-to-population transfer---whether the ensemble
values $\Prod{n}(k)$ inherit the population-level
conditional density---is captured by the
\textbf{EnsembleSelectionLemma}
(\lean{EM/Ensemble/CRT.lean}{EnsembleSelectionLemma}).
If both hypotheses hold, the total error in
$\mathbb{E}[1/\seq{\cdot}(k)]$ is bounded by a
convergent sum $\sum_q O(1/q^3) < \infty$, giving
SMLB with a universal constant:
$\mathrm{MFREConditional} + \mathrm{EnsembleSelection}
\Rightarrow \mathrm{SMLB}(1/6)$
(\lean{EM/Ensemble/CRT.lean}{MFRECondImpliesSMLB}).

A cleaner formulation isolates the population-level content:
\textbf{MinFacSelectionIndependence}
(\lean{EM/Ensemble/CRT.lean}{MinFacSelectionIndependence})
states that, for $q \geq 3$ and non-death classes $d_1, d_2 \neq -1$,
the conditional density of $\minFac(m{+}1) \equiv a \pmod{q}$
given $m \equiv d_i \pmod{q}$ is the same for $d_1$ and~$d_2$
asymptotically.  MSI implies MFRE\_conditional
(\lean{EM/Ensemble/CRT.lean}{MSIImpliesMFREConditional}).

The CRT independence thus operates at three levels, documented in
\lean{EM/Ensemble/CRT.lean}{crt\_blindness\_landscape}:
\begin{enumerate}[nosep]
...
\item \textbf{Orbit} ($=$ CME/DSL, open): along the specific EM orbit,
...
\begin{property}[\defname{DSL} ---
  \lean{EM/Population/TransferStrategy.lean}{Deterministic\-Stability\-Lemma}]
\label{def:dsl}
...
\begin{leancode}
-- DSL: population equidistribution transfers to the orbit
def DeterministicStabilityLemma : Prop :=
  PopulationEquidist -> ConditionalMultiplierEquidist

-- The master chain (one open hypothesis, one ANT input)
theorem full_chain_dsl
    (h_equidist : forall (q : Nat), Nat.Prime q -> MinFacResidueEquidist q)
...
\item \textbf{ANT $\Rightarrow$ PE}: MinFacResidueEquidist (a standard
  ...
  PopulationEquidist
  (\lean{EM/Population/Proof.lean}{pe\_of\_equidist}).
...
subsumed by DSL
(\lean{EM/Transfer/CRTPointwise.lean}{dsl\_implies\_crt\_bridge}).
\end{remark}
...
(\code{Reduction/SelfCorrecting.lean}).  Define
\[
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{gen\_hitting\_implies\_gen\_mc\_proved}]
\label{thm:gen-mc}
...
(\lean{EM/Ensemble/PT.lean}{gen\_captures\_target}), paralleling the
...
\begin{theorem}[\defname{WeylHittingBridge} ---
  \lean{EM/Ensemble/PT.lean}{weyl\_hitting\_bridge\_proved}]
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{cancel\_weyl\_implies\_mc}]
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{per\_chi\_cancellation\_bridge\_proved}]
\label{thm:per-chi}
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{ensemble\_pt\_master\_simplified}]
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{jse\_transfer\_implies\_mc}]
...
The DSLVariance framework (\code{Reduction/DSLVariance.lean}) provides an
...
\begin{theorem}[\lean{EM/Reduction/DSLVariance.lean}{populationCharEnergy\_nonneg}]
$\mathcal{E} \geq 0$.
\end{theorem}
...
\begin{theorem}[\lean{EM/Reduction/DSLVariance.lean}{crossTermPair\_comm}]
...
\begin{theorem}[\lean{EM/Reduction/DSLVariance.lean}{stepDecorrelation\_implies\_pairwiseVanishing}]
$\mathrm{SD} \;\Longrightarrow\; \mathrm{PCV}$.
\end{theorem}

\begin{theorem}[\lean{EM/Reduction/DSLVariance.lean}{charSumVarianceBound\_implies\_secondMomentBound}]
...
(\lean{EM/Reduction/Master.lean}{sve\_iff\_ve}): subquadratic visit
...
The file \code{Reduction/TailWindow.lean} (477~lines, zero
...
\paragraph{The Reduction Landscape}
\label{sec:landscape}
...
\begin{theorem}[\lean{EM/Ensemble/PT.lean}{dsl\_closes\_all}]
