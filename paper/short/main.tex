...
$T(n)=n\cdot\lpf(n{+}1)$ on squarefree integers (multiply the bag of primes
collected so far by the least prime factor of the product plus one), so
the sequence is the orbit of~$2$ under~$T$ and MC asks whether that orbit
meets every prime.  We record what is provable about one orbit of~$T$, all of it
...
equivalences, not reductions: each is quantified over missing primes and
MC makes that quantifier empty; their content is the direction that turns
a hitting problem into an equidistribution problem.  (ii)~\emph{The
...
prime factor is equivalent to a family of series identities that
Dirichlet's theorem does not touch and that fails by head domination.  On
the progression $m\equiv1\pmod P$ that contains the Euclid numbers, the
least prime outside the bag is selected with relative density exactly
$1/q$---Shanks' heuristic made precise, and the $n=0$ case of an exact,
type-conditional selection law for the greedy map.  Iterating that law
...
which the seeds coprime to~$q$ whose greedy orbit misses~$q$ in its
first~$n$ multipliers have upper natural density at most~$\varepsilon$:
unconditional, with no character sums, and silent about the orbit of~$2$.
Simultaneity in~$q$ is an additivity question rather than a rate question:
it stays open for natural density past finite sets of primes, while on the
profinite ensemble $\prod_r\ZZ/r\ZZ$ with the product uniform measure the
...
propagating congruence invariant can block a prime.
\end{abstract}

\section{Introduction}
\label{sec:intro}

Euclid's proof that there are infinitely many primes takes a finite set of
primes and factors the product plus one.  Mullin~\cite{Mullin1963} made
the choice canonical: always take the \emph{smallest} prime factor.
...
\subsection{The seed-average law: almost all seeds capture $q$}
...
density}, since natural density is only finitely additive: finite
subadditivity does give one
horizon uniform over any \emph{finite} set of primes
(\lean{EM/Population/AlmostAllDensity.lean}{finite\_simultaneous\_density}),
and that is as far as it goes.  It is proved on a different ensemble,
...
$\Omega=\prod_{r}\ZZ/r\ZZ$ over all primes with $\mu$ the product of the
uniform measures.  Coordinates $\ZZ/r\ZZ$, not $\ZZ_r$: the programme only
...
$\mu$-null set can have upper density~$1$ in~$\NN$ --- and transfer must go
