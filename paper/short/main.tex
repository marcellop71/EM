...
The branch is not empty in the comparison class.  Over $\FF_5[t]$, with
$t$ in place of~$2$ and least-degree irreducible factor in place of
$\lpf$, every Euclid polynomial of the greedy sequence is irreducible
(\lean{EM/FunctionField/StableTower.lean}{tower\_euclid\_irreducible}):
the sequence is the Sylvester tower $t,\,t+1,\,t^2+t+1,\,
t^4+2t^3+2t^2+t+1,\dots$, one irreducible of each degree~$2^k$, and the
function-field Mullin conjecture is \emph{false} there
(\lean{EM/FunctionField/StableTower.lean}{not\_ffMullinConjecture\_five}).
The proof is a two-element check---the orbit of $-1/4=1$ under
$w\mapsto w^2+w$ in $\FF_5$ is $1,2,1,2,\dots$ and $3,2$ are non-squares---
plus the observation that the Frobenius orbit of a root of the $n$-th level
polynomial has exactly $2^n$ points.  Reducing mod~$5$ also shows that
every level polynomial $\Phi_6^{\,n}+1$ is irreducible over~$\mathbb{Q}$
(\lean{EM/FunctionField/GenericTower.lean}{gQ\_irreducible}): the
\emph{generic} sequence, seeded at the indeterminate~$x$, is a Sylvester
tower that never leaves, the function-field sequences are its reductions,
and the integer sequence is its specialization at $x=2$, which leaves the
tower at stage~$3$ because $1807 = 13\cdot139$.  In the other direction
the floor is a theorem over $\FF_2[t]$, where $P\mapsto P^2+P$ is additive
and no four consecutive Euclid polynomials are irreducible
(\lean{EM/FunctionField/CompositeFloors.lean}{ffGrowthConstant\_eq\_zero\_of\_two}),
and for $p\equiv1\pmod 3$, where $w^2+w+1$ splits and no two are
(\lean{EM/FunctionField/CompositeFloors.lean}{ffGrowthConstant\_eq\_zero\_of\_one\_mod\_three});
over $\FF_3$, where $w^2+w+1=(w-1)^2$, an irreducible Euclid polynomial is
followed by a perfect square
(\lean{EM/FunctionField/CharThree.lean}{euclid\_succ\_eq\_sq}).
Over $\ZZ$ the statement that some Sylvester tower of Euclid numbers is
all prime is what $\Cinf$ denies and what no method can exclude.

The conjecture itself is the top of this ladder of ``the least factor is
small''.  With $h(n)$ the least prime not yet selected, $a(n{+}1) =
\lpf(P_n+1) \ge h(n)$ always, and
$\MC \iff h(n)\to\infty \iff \lpf(P_n+1) = h(n)$ infinitely often
$\iff \lpf(P_n+1) \le f(h(n))$ for some function~$f$
(\lean{EM/Population/HeadDynamics.lean}{mullin\_iff\_exists\_bound}): MC
fails exactly when the head stalls while the multipliers, being distinct
primes, escape to infinity.  Nothing weaker than equality with the head
suffices.  Every head capture after stage~$0$ is a composite Euclid number
(\lean{EM/Population/HeadDynamics.lean}{capture\_composite}), so MC forces
infinitely many Euclid numbers to have a prime factor below the $(n{+}2)$-th
prime, i.e.\ below $(1+o(1))\log(P_n+1)$
...
In head coordinates (the head of a seed being the least prime outside its
bag) the simultaneous law is exactly the statement that a head which has
passed~$Q$ rarely stalls afterwards
(\lean{EM/Population/SeedHead.lean}{headEscapesAA\_iff\_stallTail}), and
effective per-prime tails can give an effective growing range but not the
simultaneous law: valid at all scales they already imply a cofinite Mullin
conjecture (\lean{EM/Population/SeedHead.lean}{allScaleTail\_cofinite\_mc}).
That is as far as it goes, and the only bridge to a single orbit is a
theorem: if some relation on seeds preserves ``$m$ misses~$q$'' for every
$q$ and its class of~$2$ has positive upper density, then $\MC$ holds
(\lean{EM/Population/GrandOrbit.lean}{transfer\_principle}); for the
natural relation ``orbits eventually coincide'' the class is presumably
thin.  It is proved on a different ensemble,
...
with the $\FF_5[t]$ refutation and with placeholders marked), the
spectral and variance routes, the
