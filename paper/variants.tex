...
\paragraph{The $\Phi_3$ exclusion criterion.}
Under perpetual irreducibility of $F(n)+1$ (the FF analog of perpetual
primality), the walk position mod a degree-$1$ target follows the
autonomous map $f(w) = w(w{+}1)$ on~$\FF_p$.  The preimage of~$-1$
under~$f$ is exactly the root set of $\Phi_3(w) = w^2{+}w{+}1$
(\lean{EM/FunctionField/AutonomousMap.lean}{ffAutonomousMap\_eq\_neg\_one\_iff}
{\color{leanink}$\checkmark$}).

\begin{theorem}[$\Phi_3$ exclusion ---
\lean{EM/FunctionField/AutonomousMap.lean}{ff\_neg\_one\_unreachable}
{\color{leanink}$\checkmark$}]
\label{thm:ff-phi3-exclusion}
For $p \equiv 2 \pmod{3}$ with $p \geq 5$: $\Phi_3$ has no roots
in~$\FF_p$
(\lean{EM/FunctionField/AutonomousMap.lean}{phi3\_no\_roots}
{\color{leanink}$\checkmark$}),
so $-1$ has no preimage under~$f$.  Consequently, the orbit from any
starting value $a \neq -1$ \textbf{never reaches~$-1$}.
\end{theorem}

\noindent
The proof uses Lagrange's theorem: a root of $\Phi_3$ gives an element
of order~$3$ in~$\FF_p^\times$, forcing $3 \mid p{-}1$; but
$p \equiv 2 \pmod{3}$ gives $p{-}1 \equiv 1 \pmod{3}$, a
contradiction.  This is a structural obstruction for degree-$1$ capture
in FF-EM: the value $-1$ is absorbing (maps to~$0$, then fixed),
and unreachable from any other starting point.
The formalization (\code{FunctionField/AutonomousMap.lean}, 225~lines,
zero \code{sorry}) also proves $f(0) = 0$ (fixed point), $f(-1) = 0$
(absorbing), and that~$-1$ is visited at most once (only at step~$0$
if the starting value is~$-1$ itself).
This parallels the integer-side orbit exclusions
(Theorem~\ref{thm:perpetual-periodicity}, items~4--7) but is
\emph{stronger}: over~$\ZZ$, specific primes are excluded one at a
time by checking closed orbits; over~$\FF_p[t]$, the $\Phi_3$
criterion excludes \emph{all} degree-$1$ targets simultaneously
for $p \equiv 2 \pmod{3}$.

\paragraph{The factor tree and stochastic FF-MC.}
The integer-side factor tree framework (\S\ref{sec:factor-tree})
transfers directly to $\FF_p[t]$.  The factor tree
(\code{FunctionField/FactorTree.lean}, 455~lines, zero \code{sorry})
defines \defname{FFMixedSelection}---at each step, choose \emph{any}
monic irreducible factor of $F(n)+1$, not just the min-degree one---and
proves the structural analogue of the integer-side factor tree:

\begin{theorem}[FF factor tree structure ---
\lean{EM/FunctionField/Master.lean}{theorem\_A\_factor\_tree}
{\color{leanink}$\checkmark$}]
\label{thm:ff-factor-tree}
For any valid mixed selection~$\sigma$ from a monic irreducible starting
polynomial over~$\FF_p$:
\begin{enumerate}[nosep]
\item \textbf{Selection injectivity}: no monic irreducible appears twice
  (\lean{EM/FunctionField/FactorTree.lean}{ffMixedSel\_injective}
  {\color{leanink}$\checkmark$}).
\item \textbf{Coprimality cascade}: no irreducible divides both $F(n)$
  and $F(n)+1$
  (\lean{EM/FunctionField/FactorTree.lean}{ffMixedWalkProd\_coprime\_succ}
  {\color{leanink}$\checkmark$}).
\item \textbf{Start not capturable}: the starting polynomial can never
  appear as a selected factor
  (\lean{EM/FunctionField/FactorTree.lean}{start\_not\_capturable}
  {\color{leanink}$\checkmark$}).
\item \textbf{Degree growth}: $\deg(F_\sigma(n)+1) \to \infty$
  (\lean{EM/FunctionField/PopulationEquidist.lean}{ff\_factor\_pool\_degree\_grows}
  {\color{leanink}$\checkmark$}).
\item \textbf{Accumulated product injectivity}: distinct steps give
  distinct products
  (\lean{EM/FunctionField/FactorTree.lean}{ffMixedWalkProd\_injective}
  {\color{leanink}$\checkmark$}).
\item \textbf{Infinite exploration}: the walk explores infinitely many
  distinct monic irreducibles
  (\lean{EM/FunctionField/StochasticMC.lean}{ff\_mixed\_explores\_infinitely\_many}
  {\color{leanink}$\checkmark$}).
\end{enumerate}
All six clauses are unconditional---zero open hypotheses.
\end{theorem}

\begin{theorem}[Finiteness of irreducibles per degree ---
\lean{EM/FunctionField/Finiteness.lean}{ffFiniteIrreduciblesPerDegree\_proved}
{\color{leanink}$\checkmark$}]
\label{thm:ff-finiteness}
For any prime~$p$ and any degree~$d$, the set of monic irreducible
polynomials of degree~$d$ over~$\FF_p$ is finite.
\end{theorem}

\noindent
The proof injects monic degree-$d$ polynomials into
$\operatorname{Fin}(d{+}1) \to \FF_p$ via the coefficient map;
irreducibles form a subset, hence are also finite.
This eliminates finiteness as an open hypothesis from the entire FF-MC
chain:

\begin{theorem}[Unconditional FF bootstrap ---
\lean{EM/FunctionField/Finiteness.lean}{ff\_dh\_implies\_ffmc\_unconditional}
{\color{leanink}$\checkmark$}]
\label{thm:ff-bootstrap-uncond}
$\mathrm{FFDynamicalHitting} \;\Longrightarrow\; \mathrm{FF\text{-}MC}$.
No hypothesis beyond FFDH is required: finiteness is supplied
unconditionally by Theorem~\textup{\ref{thm:ff-finiteness}}.
\end{theorem}

The stochastic FF-MC assembles these ingredients:

\begin{keyresult}
\begin{theorem}[Stochastic FF-MC ---
\lean{EM/FunctionField/StochasticMC.lean}{stochastic\_mc\_unconditional}]
\label{thm:ff-stochastic-mc}
With any positive probability of non-greedy factor selection at each
step, every monic irreducible polynomial over~$\FF_p$ is eventually
captured.  That is, $\mathrm{FFStochasticMC}$ holds for all
$\varepsilon > 0$.
\end{theorem}
\end{keyresult}

\begin{theorem}[FF phase transition ---
\lean{EM/FunctionField/StochasticMC.lean}{ff\_phase\_transition\_unconditional}
{\color{leanink}$\checkmark$}]
\label{thm:ff-phase-transition}
The transition is sharp at $\varepsilon = 0$:
\begin{enumerate}[nosep]
\item For $\varepsilon > 0$: the stochastic process captures every monic
  irreducible (Theorem~\textup{\ref{thm:ff-stochastic-mc}}).
\item For $\varepsilon = 0$: each nontrivial character value has norm
  exactly~$1$ (no spectral contraction); the walk reduces to the open
  FF-MC conjecture.
\end{enumerate}
The standard FF-MC (the deterministic greedy walk) lives precisely at the
$\varepsilon = 0$ critical point.  Unconditional---zero open hypotheses.
\end{theorem}

\paragraph{Necklace identity and almost-all GenMixedMC.}
Over~$\FF_p[t]$, the \emph{necklace identity}
\[
  \sum_{d \mid n} d \cdot \pi_p(d) \;=\; p^n
  \qquad (n \geq 1)
\]
is proved by Galois theory: every element of $\FF_{p^n}$ is a root of
its minimal polynomial, whose degree divides~$n$, and each monic
irreducible of degree~$d$ has exactly~$d$ roots in~$\FF_{p^n}$.
The formalization (\code{FunctionField/NecklaceFormula.lean}, 446~lines,
zero~\code{sorry}) proves the prerequisite
$|\{\text{monic degree-}n \text{ polys over } \FF_p\}| = p^n$
via the coefficient equivalence
$\code{monicEquivDegreeLT} \circ \code{degreeLTEquiv}$, then
derives the necklace identity by combinatorial accounting.

\begin{theorem}[Necklace identity ---
\lean{EM/FunctionField/NecklaceFormula.lean}{necklace\_identity\_proved}
{\color{leanink}$\checkmark$}]
\label{thm:necklace-identity}
For every prime~$p$ and every $n \geq 1$,
$\sum_{d \mid n} d \cdot \pi_p(d) = p^n$,
where $\pi_p(d)$ is the number of monic irreducible polynomials
of degree~$d$ over~$\FF_p$.
Unconditional---zero open hypotheses.
\end{theorem}

From the necklace identity, the entire analytic number theory chain
over~$\FF_p[t]$ becomes unconditional:
\begin{itemize}[nosep]
\item \textbf{Character cancellation} (necklace identity):
  $\sum_{d \mid n} d \cdot \pi_p(d) = p^n$.
\item \textbf{PNT-in-APs}: every degree $d \geq 1$ has at least one
  monic irreducible ($\pi_p(d) \geq 1$).
\item \textbf{Forbidden class divergence}: the cumulative irreducible
  supply grows at least linearly ($\sum_{d=1}^{D} \pi_p(d) \geq D$).
\item \textbf{Sieve product vanishing}: $d \cdot \pi_p(d) \leq p^d$
  (from the single-term extraction of the necklace identity).
\item \textbf{PSCD}: $\pi_p(d) \leq p^d / d$.
\end{itemize}
Combined with the factor tree sieve framework, this yields:

\begin{keyresult}\label{thm:ff-almost-all}
Almost-all GenMixedMC over $\FF_p[t]$
(\lean{EM/FunctionField/FFSieve.lean}{ff\_almost\_all\_unconditional}):
for every prime~$p$, a density-$1$ set of squarefree monic polynomials
$m \in \FF_p[t]$ satisfies $\mathrm{GenMixedMC}(m)$---every monic
irreducible appears somewhere in the factor tree~$\mathcal{T}(m)$.
\textbf{Unconditional---zero open hypotheses, machine-verified.}
\end{keyresult}

\noindent
The comparison with the integer case isolates the gap:
...
\begin{tabular}{lcc}
\toprule
\textbf{Step} & \textbf{Over $\ZZ$} & \textbf{Over $\FF_p[t]$} \\
\midrule
Character cancellation & Requires GRH & Exact (necklace) \\
PNT-in-APs & Requires Siegel--Walfisz & Exact ($\pi_p(d) \geq 1$) \\
Forbidden class divergence & Requires WPNT-in-APs & Unconditional \\
Sieve product vanishing & From FCD $+$ sparse contraction & Unconditional \\
PSCD (confined density $\to 0$) & From FCD $+$ sieve product & Unconditional \\
Almost-all GenMixedMC & From PSCD $+$ pigeonhole & Unconditional \\
\midrule
\textbf{Residual gap} & \multicolumn{2}{c}{orbit-specificity transfer (DSL)} \\
\bottomrule
\end{tabular}
\end{center}
\medskip
...
\noindent
The full FF unconditional series totals ${\sim}2{,}700$ new lines
across eight files (\code{NecklaceFormula}, \code{FFCharacterSums},
\code{FFSieve}, \code{FactorTree}, \code{PopulationEquidist},
\code{StochasticMC}, \code{Finiteness}, \code{Master}), all zero
\texttt{sorry}.

\begin{keyresult}
The FF unconditional landscape
(\lean{EM/FunctionField/Master.lean}{ff\_unconditional\_grand\_summary}
{\color{leanink}$\checkmark$})
assembles:
\[
  \underbrace{\text{Necklace}}_{\text{Thm~\ref{thm:necklace-identity}}}
  \;+\;
  \underbrace{\text{a.a.\ GenMixedMC}}_{\text{Thm~\ref{thm:ff-almost-all}}}
  \;+\;
  \underbrace{\text{Factor tree}}_{\text{Thm~\ref{thm:ff-factor-tree}}}
  \;+\;
  \underbrace{\text{Finiteness}}_{\text{Thm~\ref{thm:ff-finiteness}}}
  \;+\;
  \underbrace{\text{Stochastic MC}}_{\text{Thm~\ref{thm:ff-stochastic-mc}}}
  \;+\;
  \underbrace{\text{Phase transition}}_{\text{Thm~\ref{thm:ff-phase-transition}}}
\]
into a single machine-checked conjunction.  Every clause is
unconditional (zero open hypotheses).
The sole residual for the deterministic FF-MC is FFDH
(Theorem~\textup{\ref{thm:ff-bootstrap-uncond}}).
\end{keyresult}
...
The formalization across seventeen files (${\sim}7{,}400$~lines, zero
