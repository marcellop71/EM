...
The proved chain so far:
TreeContractionAtHalf $\xRightarrow{\checkmark}$ vanishing
$\mathrm{pathCharSum}$
(\lean{EM/Advanced/RandomTwoPointMC.lean}{pathCharSum\_vanishing\_of\_tree\_contraction}
{\color{leanink}$\checkmark$}).
The remaining gap from vanishing path character sums to positive
path counts is identified as \defname{ChiAtMultiplicativity}:
that the product of per-step $\mathrm{chiAt}$ values equals
$\mathrm{chiAt}$ of the walk endpoint, which holds when each factor
is coprime to~$q$.
...
$\chi$ maps to $\{1, -1\}$).  This is the structural reason why
$q = 3$ is the most favorable case: every diverse step contributes
zero to the character average, giving spectral gap~$1$.
