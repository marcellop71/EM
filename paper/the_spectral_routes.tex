...
Every prime \emph{not dividing~$n$} appears in the generalized EM
sequence from starting point~$n$: $\forall\, q$ prime with $q \nmid n$,
$\exists\, k$, $\seq{n}(k) = q$.  The restriction is forced: $n \mid
\Prod{n}(k)$ for all~$k$, so a prime dividing~$n$ is coprime to
$\Prod{n}(k)+1$ and is never selected (dead end~\#175,
\lean{EM/Ensemble/WeylChain.lean}{gen\_mc\_unrestricted\_false}).
\end{definition}
...
\begin{proof}
Strong induction on~$q$.  Assume every prime $p < q$ with $p \nmid n$
appears as $\seq{n}(k_p)$ for some~$k_p$.  Collect all witnesses into
a uniform bound~$N$.  Cofinal hitting at~$q$ provides $k \geq N$ with
$q \mid \Prod{n}(k) + 1$.  Every prime $p < q$ divides $\Prod{n}(k)$:
either $p \mid n$, or $p$ appeared at a step $< k$; in both cases $p$
cannot divide $\Prod{n}(k) + 1$.  So $q = \minFac(\Prod{n}(k) + 1) =
\seq{n}(k)$.
