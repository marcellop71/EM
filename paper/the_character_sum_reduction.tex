...
Section~\ref{sec:walk} established the picture: for the first
...
$f(n) = -\walkZ{2}{q}{n}^{-1}$ is dodged at every step.  The Single Hit
Theorem (Theorem~\ref{thm:single-hit}) reduces MC to producing one hit
...
$|\{n < N : \walkZ{2}{q}{n} = -1\}|$ stays at zero past the sieve gap.
...
sums $S_\chi(N) = \sum_{n<N}\chi(\walkZ{2}{q}{n})$.  The uniform share
...
\item[\S4.1] Character sums and permanent avoidance --- orthogonality detects the anomaly; CCSB defined.
\item[\S4.2] The Fourier bridge --- hit count $=$ uniform share $+$ character corrections; CCSB $\Rightarrow$ hits.
\item[\S4.3] Decorrelation--PED--CCSB chain --- conditional multiplier equidistribution implies character cancellation.
\item[\S4.4] Vanishing conditional bias --- walk telescoping reduces to one-step conditional bias.
\item[\S4.5] Walk telescoping identities --- structural identities for character energy.
\end{enumerate}
\end{roadmap}
...
\[
  S_\chi(N) \;=\; \sum_{n < N} \chi\bigl(\walkZ{2}{q}{n}\bigr).
\]

\begin{property}[\defname{ComplexCharSumBound} (\abbr{CCSB})]
...
\begin{leancode}
-- The hypothesis: all character sums are o(N)
def ComplexCharSumBound : Prop :=
  forall (q : Nat) (hq : Nat.Prime q) (hne : forall k, seq k != q),
  forall (chi : (ZMod q)^x ->* C^x) (_hchi : chi != 1),
  forall (eps : R) (_heps : 0 < eps),
  exists N_0, forall N >= N_0,
    ||sum n in Finset.range N, (chi (emWalkUnit q hq hne n) : C)|| <= eps * N

-- The reduction: one line
...
\item \textbf{Fourier inversion} (\lean{EM/Equidist/FourierB.lean}{complex\_csb\_implies\_hit\_count\_lb\_proved}):
  CCSB implies that the hit count at~$-1$ satisfies
  $|\{n < N : \walkZ{2}{q}{n} = -1\}| = N/(q{-}1) + o(N)$,
...
\item \textbf{Cofinal hitting gives HH}
  (\lean{EM/Equidist/Fourier.lean}{walk\_equidist\_mc}):
...
\begin{theorem}[\lean{EM/Equidist/FourierB.lean}{walk\_hit\_count\_fourier\_step}]
\label{thm:fourier-step}
For any target $t \in (\ZZ/q\ZZ)^\times$:
\[
  \bigl|\{n < N : \walkZ{2}{q}{n} = t\}\bigr|
...
Take as a running example $q = 41$, the smallest prime not yet known
to appear in the sequence: the walk must accumulate
...
from \emph{multipliers}: $\walkZ{2}{q}{n{+}1} = \walkZ{2}{q}{n} \cdot
\multZ{2}{q}{n}$.  Since $\chi$ is a group homomorphism,
$\chi(\walkZ{2}{q}{n}) = \chi(\walkZ{2}{q}{0}) \cdot \prod_{k<n}
\chi(\multZ{2}{q}{k})$: the walk character sum is a sum of
...
\begin{property}[\defname{PositiveEscapeDensity} (\abbr{PED})]
...
$\ker(\chi)$ ``rotate'' the walk character value $\chi(\walkZ{2}{q}{n})$
...
\begin{property}[\defname{DecorrelationHypothesis}]
For every missing prime~$q$ and non-trivial~$\chi$, the multiplier
character sums are $o(N)$:
$\|\sum_{n<N} \chi(\multZ{2}{q}{n})\| \leq \varepsilon N$ for large~$N$.
\end{property}
...
\begin{property}[\defname{NoLongRuns}$(L)$]
...
\begin{property}[\defname{BlockRotationEstimate} (\abbr{BRE})]
...
``rotates'' the walk character value $\chi(\walkZ{2}{q}{n})$ by a
...
multiplier: the death channel $-\walkZ{2}{q}{n}^{-1}$ is a function of
...
$\|\sum_{\substack{n < N \\ \walkZ{2}{q}{n} = c}} \chi(\multZ{2}{q}{n})\|
\leq \varepsilon N$.
\end{property}
...
Formally, CME implies Decorrelation (and, being quantified over missing
primes, is equivalent to MC, \lean{EM/CME/Equivalences.lean}{cme\_iff\_mc};
Decorrelation is also implied by MC vacuously, and whether Decorrelation
implies MC is open): it bounds the
fiber sums
$\sum_{\substack{n<N \\ \walkZ{2}{q}{n} = c}} \chi(\multZ{2}{q}{n}) = o(N)$
...
(\lean{EM/CME/Reduction.lean}{cme\_implies\_dec}).
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{decorrelation\_implies\_ped}]
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{noLongRuns\_implies\_ped}]
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{block\_rotation\_implies\_ped\_csb}]
...
(\lean{EM/CME/Reduction.lean}{cme\_implies\_ccsb}):
\[
\mathrm{CME}
\;\xrightarrow{\text{proved}}\;
\mathrm{CCSB}
...
\begin{property}[\defname{VanishingConditionalBias} (\abbr{VCB})]
\label{def:vcb}%
...
\begin{proposition}[\lean{EM/CME/FiberAnalysis.lean}{cme\_implies\_vcb}]
...
\begin{theorem}[\lean{EM/CME/FiberAnalysis.lean}{vcb\_ped\_implies\_ccsb}]
...
(\lean{EM/CME/FiberAnalysis.lean}{norm\_sub\_one\_sq\_eq},
\lean{EM/CME/FiberAnalysis.lean}{unit\_norm\_re\_le\_of\_dist}).
...
\begin{theorem}[\lean{EM/CME/FiberAnalysis.lean}{vcb\_ped\_implies\_mc}]
...
\paragraph{Fiber Energy Analysis}
\label{sec:fiber-energy}

The fiber decomposition of CME admits a quantitative refinement via
character energy.  For a character~$\chi$ and walk of length~$N$,
define the \emph{character energy}
\[
  E_\chi(N) \;=\; \Bigl\|\sum_{n<N} \chi(\walkZ{2}{q}{n})\Bigr\|^2,
\]
which decomposes as
\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{char\_sum\_energy\_eq\_N\_plus\_cross}]
$E_\chi(N) = N + 2\sum_{j<k<N} \mathrm{Re}\bigl(
  \overline{\chi(\walkZ{2}{q}{j})} \cdot \chi(\walkZ{2}{q}{k})\bigr)$.
\end{theorem}
\noindent If the cross terms are $o(N)$, the energy grows linearly
rather than quadratically:
\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{cross\_term\_controls\_growth}]
$o(N)$ cross terms $\Rightarrow$ $E_\chi(N) = N + o(N^2)$,
i.e., the walk character sum is $o(N)$.
\end{theorem}

The cross terms admit a fiber decomposition indexed by walk position:
\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{total\_cross\_term\_eq\_sum\_fiber}]
The total cross term equals the sum of \emph{fiber cross terms} over
walk positions~$a \in (\ZZ/q\ZZ)^\times$.
\end{theorem}

This connects to CME via the \emph{Fiber Energy Bound}~(FEB),
which asks that the sum of squared fiber norms
$\sum_a \|F(a,N)\|^2 \leq \varepsilon N^2$:

\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{feb\_implies\_cme}]
$\mathrm{FEB} \;\Longrightarrow\; \mathrm{CME}$.
\end{theorem}

\begin{proof}[Proof sketch]
Each individual fiber satisfies $\|F(a)\|^2 \leq \sum_{a'} \|F(a')\|^2
\leq \varepsilon N^2$, so $\|F(a)\| \leq \sqrt{\varepsilon}\, N$.
This holds for all~$a$, giving CME with error $\sqrt{\varepsilon}\, N$.
\end{proof}

The fiber energy perspective also clarifies that character values on
$(\ZZ/q\ZZ)^\times$ have unit norm:

\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{char\_value\_norm\_one}]
For any finite group $G$ and character $\chi : G \to \mathbb{C}^\times$,
$\|\chi(g)\| = 1$ for all $g \in G$.
\end{theorem}

\noindent This is used throughout the energy estimates: since each term
in the walk character sum has norm~$1$, the energy~$E_\chi(N)$ lies
between~$N$ (perfect cancellation) and~$N^2$ (no cancellation).
CME via FEB asks that the energy is close to the cancellation end.
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{walk\_telescope\_identity}]
\label{thm:telescope}
For any $\chi$ and $N$:
\[
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{walk\_telescope\_norm\_bound}]
...
\begin{theorem}[\lean{EM/Equidist/SelfCorrecting.lean}{walk\_shift\_one\_correlation}]
\label{thm:shift-one}
...
\paragraph{The cofactor identity.}
A finer structural identity arises from the ``$+1$ shift'' in the
Euclid construction.  Define the \emph{cofactor}
$\text{cof}(n) = (\Prod{2}(n)+1)/\seq{2}(n{+}1)$, and its residue
$\mathrm{cofZ}(q, n) = \text{cof}(n) \bmod q$.  Then:

\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{shifted\_walk\_eq\_mult\_mul\_cof}]
\label{thm:cofactor}
$\walkZ{2}{q}{n} + 1 = \multZ{2}{q}{n} \cdot \mathrm{cofZ}(q, n)$.
\end{theorem}

This factors the walk recurrence through the cofactor:
$\chi(\walkZ{2}{q}{n} + 1) = \chi(\multZ{2}{q}{n}) \cdot \chi(\mathrm{cofZ}(q,n))$.
Since the walk hits~$-1$ exactly when $\walkZ{2}{q}{n} + 1 = 0$, and
$\multZ{2}{q}{n} \neq 0$ (as a unit):

\begin{theorem}[\lean{EM/Reduction/DSLInfra.lean}{walkZ\_eq\_neg\_one\_iff\_cofZ\_zero}]
$\walkZ{2}{q}{n} = -1 \;\Longleftrightarrow\; \mathrm{cofZ}(q, n) = 0$.
\end{theorem}

The cofactor identity is unprecedented in published work on the
Euclid--Mullin sequence.  However, it does not escape the orbit-specificity
barrier: the cofactor $\mathrm{cofZ}(q, n)$ is a \emph{joint} quantity
depending on both the walk state and the factorization outcome, so the
identity factorizes one correlated variable into two correlated
variables.  This is formalized as a dead end (Dead End~\#115).

%% =========================================================================
\subsection{Sieve Routes, in Brief}
\label{sec:sieve-routes}
%% =========================================================================

Three further reductions run through sieve machinery rather than through
the telescoping identity.  Each is machine-verified as a reduction, and in
each the sole open content is the same orbit-specificity transfer; the
details are in Appendix~\ref{app:routes}.

\paragraph{The Large Sieve Route}
...
(\code{EM/LargeSieve/Basic.lean}, \code{EM/LargeSieve/Harmonic.lean},
\code{EM/LargeSieve/Analytic.lean}) totaling ${\sim}5{,}870$~lines,
...
\begin{property}[\defname{MultiModularCSB} (\abbr{MMCSB})]
...
\begin{theorem}[\lean{EM/LargeSieve/Basic.lean}{mmcsb\_implies\_mc}]
\label{thm:mmcsb-mc}
...
\begin{theorem}[\lean{EM/LargeSieve/Basic.lean}{bv\_chain\_mc}]
$\mathrm{BV} + \mathrm{BVImpliesMMCSB} \;\Longrightarrow\; \MC$.
\end{theorem}
\noindent\emph{Correction (2026-08-18).}  Until this revision the Lean
statement \code{BombieriVinogradov} was `$\exists E,\ E \le x/(\log x)^A
\wedge 0 \le E$', which is trivially true, so \code{BVImpliesMMCSB} was
literally \code{MultiModularCSB} and no BV entered the chain.  BV and EH
are now stated faithfully in the $\psi$-form
($\sum_{q \le \sqrt{x}/(\log x)^B} \max_{(a,q)=1} |\psi(x;q,a) - x/\varphi(q)|
\ll x/(\log x)^A$; \code{bvError}), and \code{BVImpliesMMCSB} is no longer
an open point of the network: the route is a dead end
(\#54, \#61, \#75, \#96), recorded, not pursued.  The transfer
hypothesis BVImpliesMMCSB would require transferring the averaged
equidistribution statement of BV
