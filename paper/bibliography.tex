%% =========================================================================
\newpage
\begin{thebibliography}{99}

\bibitem{Mullin1963}
A.\,A.~Mullin.
\newblock Recursive function theory (a modern look at a Euclidean idea).
\newblock \emph{Bull.\ Amer.\ Math.\ Soc.}, 69:737, 1963.

\bibitem{BoIr2016}
A.\,R.~Booker and S.\,A.~Irvine.
\newblock The {E}uclid--{M}ullin graph.
\newblock \emph{J.\ Number Theory}, 165:30--57, 2016.

\bibitem{BoSa2012}
A.\,R.~Booker.
\newblock A variant of the {E}uclid--{M}ullin sequence containing every prime.
\newblock \emph{J.\ Integer Sequences}, 19:Article 16.6.4, 2016.

\bibitem{Shanks1991}
D.~Shanks.
\newblock Euclid's primes.
\newblock \emph{Bull.\ Inst.\ Combin.\ Appl.}, 1:33--36, 1991.

\bibitem{BoSi2026}
A.\,R.~Booker and O.~Simon.
\newblock A generalisation of the {E}uclid--{M}ullin sequences.
\newblock \emph{arXiv preprint}, 2601.21901, 2026.

\bibitem{Gordon1961}
B.~Gordon.
\newblock Sequences in groups with distinct partial products.
\newblock \emph{Pacific J.\ Math.}, 11(4):1309--1313, 1961.

\bibitem{Booker2012}
A.\,R.~Booker.
\newblock On {M}ullin's second sequence of primes.
\newblock \emph{Integers}, 12A:A4, 2012.

\bibitem{PollackTrevino2014}
P.~Pollack and E.~Trevi{\~n}o.
\newblock The primes that {E}uclid forgot.
\newblock \emph{Amer.\ Math.\ Monthly}, 121(5):433--437, 2014.

\bibitem{Hardy2009}
M.~Hardy and C.~Woodgold.
\newblock Prime simplicity.
\newblock \emph{Math.\ Intelligencer}, 31:44--52, 2009.

\bibitem{Hooley1967}
C.~Hooley.
\newblock On {A}rtin's conjecture.
\newblock \emph{J.\ Reine Angew.\ Math.}, 225:209--220, 1967.

\bibitem{LagariasOdlyzko1977}
J.\,C.~Lagarias and A.\,M.~Odlyzko.
\newblock Effective versions of the {C}hebotarev density theorem.
\newblock In \emph{Algebraic Number Fields ({D}urham Symposium)},
  pages 409--464. Academic Press, 1977.

\bibitem{Alladi1977}
K.~Alladi.
\newblock Duality between prime factors and an application to the prime
  number theorem for arithmetic progressions.
\newblock \emph{J.\ Number Theory}, 9(4):436--451, 1977.

\bibitem{AhoSloane1973}
A.\,V.~Aho and N.\,J.\,A.~Sloane.
\newblock Some doubly exponential sequences.
\newblock \emph{Fibonacci Quart.}, 11(4):429--437, 1973.

\bibitem{Vardi1991}
I.~Vardi.
\newblock Are all {E}uclid numbers squarefree?
\newblock In \emph{Computational Recreations in Mathematica}, pages 82--89.
  Addison-Wesley, 1991.

\bibitem{Hildebrand1986}
A.~Hildebrand.
\newblock On the number of positive integers $\leq x$ and free of
  prime factors $> y$.
\newblock \emph{J.\ Number Theory}, 22(3):289--307, 1986.

\bibitem{CoxVdP1968}
C.\,D.~Cox and A.\,J.~van~der~Poorten.
\newblock On a sequence of prime numbers.
\newblock \emph{J.\ Austral.\ Math.\ Soc.}, 8:571--574, 1968.

\bibitem{Kornblum1919}
H.~Kornblum.
\newblock {\"U}ber die {P}rimfunktionen in einer arithmetischen
  {P}rogression.
\newblock \emph{Math.\ Z.}, 5:100--111, 1919.

\bibitem{IwaniecKowalski2004}
H.~Iwaniec and E.~Kowalski.
\newblock \emph{Analytic Number Theory}.
\newblock Amer.\ Math.\ Soc.\ Colloq.\ Publ., vol.~53, 2004.

\bibitem{DiaconisShahshahani1981}
P.~Diaconis and M.~Shahshahani.
\newblock Generating a random permutation with random transpositions.
\newblock \emph{Z.\ Wahrsch.\ Verw.\ Gebiete}, 57:159--179, 1981.

\bibitem{ChungDiaconisGraham1987}
F.\,R.\,K.~Chung, P.~Diaconis, and R.\,L.~Graham.
\newblock Random walks arising in random number generation.
\newblock \emph{Ann.\ Probab.}, 15(3):1148--1165, 1987.

\bibitem{Sarnak2010}
P.~Sarnak.
\newblock Three lectures on the {M}\"obius function, randomness,
  and dynamics.
\newblock Lecture notes, IAS, 2010.
\newblock Available at \url{https://publications.ias.edu/sarnak/paper/512}.

\bibitem{Artin1927}
E.~Artin.
\newblock Beweis des allgemeinen {R}eziprozit\"atsgesetzes.
\newblock \emph{Abh.\ Math.\ Semin.\ Univ.\ Hambg.}, 5:353--363, 1927.

\bibitem{Odoni1985}
R.\,W.\,K.~Odoni.
\newblock On the prime divisors of the sequence $w_{n+1} = 1 + w_1 \cdots w_n$.
\newblock \emph{J.\ London Math.\ Soc.~(2)}, 32(1):1--11, 1985.

\bibitem{Tao2019Collatz}
T.~Tao.
\newblock Almost all orbits of the {C}ollatz map attain almost bounded values.
\newblock \emph{Forum Math.\ Pi}, 10:e12, 2022.

\bibitem{Deligne1980}
P.~Deligne.
\newblock La conjecture de {W}eil.~{II}.
\newblock \emph{Publ.\ Math.\ IHES}, 52:137--252, 1980.

\bibitem{KonyaginShparlinski1999}
S.\,V.~Konyagin and I.\,E.~Shparlinski.
\newblock \emph{Character Sums with Exponential Functions and Their Applications}.
\newblock Cambridge Tracts in Math., vol.~136, Cambridge Univ.\ Press, 1999.

\bibitem{CA2026}
M.~Paris.
\newblock {CA}: Content-addressing for {L}ean~4 environments.
\newblock Software library, 2026.
\newblock \url{https://github.com/mparis/CA}.

\bibitem{LeanArchitect2026}
T.~Zhu, P.~Monticone, J.~Avigad, and S.~Welleck.
\newblock {LeanArchitect}: Automating blueprint generation for humans and {AI}.
\newblock \emph{arXiv preprint}, 2601.22554, 2026.

\bibitem{Weil1948}
A.~Weil.
\newblock Sur les courbes alg\'ebriques et les vari\'et\'es qui s'en d\'eduisent.
\newblock \emph{Actualit\'es Sci.\ Ind.}, no.\ 1041, Hermann, Paris, 1948.

\bibitem{Cohen1981}
S.\,D.~Cohen.
\newblock The distribution of {G}alois groups and {H}ilbert's irreducibility theorem.
\newblock \emph{Proc.\ London Math.\ Soc.}, 43:227--250, 1981.

\bibitem{Ribet1990}
K.\,A.~Ribet.
\newblock On modular representations of
  $\mathrm{Gal}(\overline{\mathbb{Q}}/\mathbb{Q})$ arising from modular forms.
\newblock \emph{Invent.\ Math.}, 100:431--476, 1990.

\bibitem{Wiles1995}
A.~Wiles.
\newblock Modular elliptic curves and {F}ermat's last theorem.
\newblock \emph{Ann.\ of Math.}, 141(3):443--551, 1995.

\bibitem{EberlPaulsonPNT}
M.~Eberl and L.\,C.~Paulson.
\newblock The prime number theorem.
\newblock \emph{Archive of Formal Proofs}, September 2018.
\newblock Isabelle/HOL entry, theory \texttt{Mertens\_Theorems}.
\newblock \url{https://www.isa-afp.org/entries/Prime_Number_Theorem.html}.

\bibitem{PNTplus}
A.~Kontorovich, T.~Tao, et al.
\newblock {PrimeNumberTheoremAnd}.
\newblock Lean~4 project, 2024--2026; file
  \texttt{PrimeNumberTheoremAnd/IEANTN/Mertens.lean}.
\newblock \url{https://github.com/AlexKontorovich/PrimeNumberTheoremAnd}.

\bibitem{Mathlib41394}
T.~Tao.
\newblock \texttt{feat(NumberTheory/Mertens)}: the {M}ertens theorems.
\newblock mathlib4 pull request \#41394, July 2026 (open at the time of writing).
\newblock \url{https://github.com/leanprover-community/mathlib4/pull/41394}.

\bibitem{HHR2016}
M.\,A.~Hill, M.\,J.~Hopkins, and D.\,C.~Ravenel.
\newblock On the nonexistence of elements of {K}ervaire invariant one.
\newblock \emph{Ann.\ of Math.}, 184(1):1--262, 2016.

\end{thebibliography}

